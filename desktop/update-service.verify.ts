...
for (const platform of ['win32', 'linux'] satisfies NodeJS.Platform[]) {
  assert.equal(
    supportsDirectDesktopUpdates({ packaged: true, smoke: false, platform, updateConfigPresent: true }),
    true,
    `${platform} packaged builds must support direct updates`,
  );
}
assert.equal(
  supportsDirectDesktopUpdates({ packaged: true, smoke: false, platform: 'darwin', updateConfigPresent: true }),
  false,
  'ad-hoc signed macOS builds must use the release-page fallback',
);
assert.equal(
  supportsDirectDesktopUpdates({ packaged: false, smoke: false, platform: 'win32', updateConfigPresent: true }),
  false,
  'development builds must not contact update servers',
);
assert.equal(
  supportsDirectDesktopUpdates({ packaged: true, smoke: true, platform: 'linux', updateConfigPresent: true }),
  false,
  'smoke builds must not contact update servers',
);
assert.equal(
  supportsDirectDesktopUpdates({ packaged: true, smoke: false, platform: 'win32', updateConfigPresent: false }),
  false,
  'private/local YoloCut builds without their own update repository must fail closed',
);
