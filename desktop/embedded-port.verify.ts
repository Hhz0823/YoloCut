...
try {
  const migrationHome = join(home, 'migration-home');
  const legacyRoot = join(migrationHome, LEGACY_HOME_DIR_NAMES[0]);
  mkdirSync(legacyRoot, { recursive: true });
  writeFileSync(join(legacyRoot, 'mcp-port'), '6123\n');
  assert.equal(readRememberedPort({ home: migrationHome }), 6123);
  assert.equal(readFileSync(embeddedPortPath({ home: migrationHome }), 'utf8').trim(), '6123');
  ...
  assert.ok(rememberPort(6124));
  assert.equal(embeddedPortPath(), join(isolatedMachineState, 'mcp-port'));
  assert.equal(readRememberedPort(), 6124);
