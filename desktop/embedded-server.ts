...
export async function startEmbeddedServer(distDir: string): Promise<EmbeddedServer> {
  ...
  // Ordinary project-store reads/writes stay on trusted IPC in the desktop
  // shell. projectStoreHttp:false still mounts the narrowly scoped SQLite
  // migration status/run/cleanup routes used by the settings dialog.
  const fake = {
