...
const exportsDirectory = resolve('/tmp/exports');
assert.equal(exportRevealCandidate(exportsDirectory, 'demo.mp4'), join(exportsDirectory, 'demo.mp4'));
assert.equal(exportRevealCandidate(exportsDirectory, '../demo.mp4'), null, 'filenames must not escape the export directory');
assert.equal(exportRevealCandidate(exportsDirectory, resolve('/tmp/demo.mp4')), null, 'absolute filenames must be rejected');
...
assert.match(main, /yolocut:reveal-export/, 'Electron main must own the filesystem reveal operation');
...
assert.match(history, /r\.destinationId && window\.yoloCutDesktop\?\.revealExport/, 'legacy and browser rows must not offer reveal');
