...
// The embedded server installs an outbound proxy globally. Its own smoke
// probe must explicitly use a direct dispatcher or localhost checks can be
// sent through a desktop proxy and fail despite the server being healthy.
{
  const smoke = await readFile(new URL('./smoke-probe.ts', import.meta.url), 'utf8');
  ...
  assert.match(
    smoke,
    /\/api\/project-store\/migrate-status/,
    'desktop smoke must fail when the packaged migration endpoint falls through to static HTML',
  );

  const recovery = await readFile(new URL('./smoke-mcp-recovery.ts', import.meta.url), 'utf8');
  assert.match(recovery, /const LOOPBACK_DISPATCHER = new Agent\(\)/);
  assert.match(
    recovery,
    /fetch:\s*loopbackFetch/,
    'the MCP SDK transport must bypass the global outbound proxy for loopback recovery checks',
  );
}
