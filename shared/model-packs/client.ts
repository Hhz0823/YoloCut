...
/** Voice-only catalog for local TTS model/voice selection and install status. */
export async function fetchVoiceModelPackCatalog(): Promise<readonly ModelPackCatalogEntry[]> {
  const response = await fetch('/api/model-packs?kind=voice', { cache: 'no-store' });
  const body = await responseJson<CatalogResponse>(response);
  if (!Array.isArray(body.packs) || body.packs.some((pack) => pack.kind !== 'voice')) {
    throw new Error('Invalid voice model pack catalog response');
  }
  return body.packs;
}
...
export interface ModelPackInstallOptions {
  readonly licenseAcceptance?: string;
}
...
): Promise<ModelPackTask> {
  const response = await fetch('/api/model-packs/download', {
    method: 'POST',
    headers: modelPackMutationHeaders(headers),
    body: JSON.stringify({ id, ...(options.licenseAcceptance ? {
      licenseAcceptance: options.licenseAcceptance,
    } : {}) }),
  });
