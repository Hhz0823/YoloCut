 ...
 * with no vision models in the catalog yield an empty list; local/gateway providers
 * (Ollama/LM Studio/ZCode) keep their configured model so local vision models
...
export function listVisionModels(
  ...
  if (ids.length === 0 && provider !== 'ollama' && provider !== 'lmstudio' && provider !== 'zcode') return [];
...
export function resolveModelCapabilities(
  ...
  const tools = override?.supportsTools !== undefined ? exact(override.supportsTools, 'settings-override')
    : typeof model?.supportsTools === 'boolean' ? exact(model.supportsTools, 'catalog') : fallback(identity.provider === 'zcode');
  const images = override?.supportsImages !== undefined ? exact(override.supportsImages, 'settings-override')
    : model ? exact(model.input.includes('image'), 'catalog') : fallback(identity.provider === 'zcode');
