...
export const LLM_PROVIDER_PRESETS = [
  ...
  {
    // ZCode Antigravity is a separately managed loopback gateway. YoloCut
    // stores only the user-copied local API key and talks to its OpenAI-compatible
    // surface; OAuth/provider credentials remain owned by ZCode.
    id: 'zcode',
    label: 'ZCode · Antigravity (Local)',
    protocol: 'openai-compatible',
...
/** Loopback providers must never be described as remote cloud endpoints. */
export function isLoopbackLlmProvider(provider: unknown): boolean {
  const normalized = normalizeLlmProvider(provider);
  return normalized === 'zcode' || isLocalLlmProvider(normalized);
}
