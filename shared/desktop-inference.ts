...
export const DESKTOP_INFERENCE_CHANNELS = {
  capabilities: 'yolocut:inference-capabilities',
  setEnabled: 'yolocut:inference-set-enabled',
  preloadAsr: 'yolocut:inference-preload-asr',
  transcribe: 'yolocut:inference-transcribe',
  semantic: 'yolocut:inference-semantic',
  clap: 'yolocut:inference-clap',
  rhythm: 'yolocut:inference-rhythm',
  cancel: 'yolocut:inference-cancel',
  progress: 'yolocut:inference-progress',
} as const;
...
function validSourcePath(value: unknown): value is string {
  ...
  try {
    const parsed = new URL(value, 'http://yolocut.local');
    return parsed.origin === 'http://yolocut.local'
      && parsed.search === ''
      && parsed.hash === ''
