...
// Policy entries use repository-style forward slashes. Normalize Windows
// `path.relative()` output before matching so allowlists behave identically on
// every supported development platform.
const repoRelative = (filePath) => path.relative(ROOT, filePath).split(path.sep).join('/');
...
function issue(sf, node, message) {
  const relative = repoRelative(sf.fileName);
