...
//   YOLOCUT_WHISPER_CLI or copied into public/whisper-cli/darwin-arm64/.
...
const PLATFORMS = {
  'darwin-arm64': {
    ...
    note: 'compile from source; YOLOCUT_WHISPER_CLI overrides',
  },
...
async function buildFromSource(srcDir, buildDir, platformKey) {
  ...
  try {
    ...
    throw new Error('cmake is required to build whisper.cpp on this platform (brew install cmake); or set YOLOCUT_WHISPER_CLI to a prebuilt binary');
  }
...
async function main() {
  ...
  if (!spec.asset) {
    ...
    // YOLOCUT_WHISPER_CLI still wins when set.
