...
const fixture = await mkdtemp(join(tmpdir(), 'yolocut-dev-profile-'));
...
try {
  ...
  assert.equal(first.rootDir, join(stablePaths.homeDir, '.yolocut', 'dev-profiles', UUID_A));
  assert.deepEqual(
  ...
  assert.equal(childEnvironment.YOLOCUT_DEV_PROFILE_ID, UUID_A);
