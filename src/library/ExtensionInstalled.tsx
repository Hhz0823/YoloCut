...
function PackDetails({ pack, props }: { pack: InstalledPack; props: InstalledProps }) {
  ...
  return (
    <div style={{ borderTop: `1px solid ${theme.border}`, padding: '9px 11px 11px' }}>
      {pack.description && (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 4 }}>
        {pack.items.map((item) => (
          <div key={`${item.type}:${item.id}`} style={{ border: `1px solid ${theme.border}`, padding: '5px 7px', color: theme.text, fontSize: 10.5, borderRadius: 3 }}>
...
function InstalledCard({ pack, props }: { pack: InstalledPack; props: InstalledProps }) {
  ...
  const busy = props.busyId !== null;
  return (
    <article style={{ border: `1px solid ${theme.border}`, background: theme.panelAlt, borderRadius: 5, overflow: 'hidden', opacity: pack.enabled ? 1 : 0.72 }}>
