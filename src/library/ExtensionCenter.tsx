...
const REGISTRY_URL = import.meta.env.VITE_RESOURCE_REGISTRY_URL
  || 'https://yolocut.com/api/resources/extensions';
...
function CenterHeader({ tab, installedCount, onTab, onClose, showLocalInstall, onLocalInstall }: {
  ...
  const t = useT();
  return (
    <header style={{ borderBottom: `1px solid ${theme.border}`, padding: '11px 14px 0' }}>
  ...
  return (
    <section style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', background: theme.panel }}>
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: 14 }}>
        {actions.status && <div style={{ marginBottom: 10, padding: '7px 9px', border: `1px solid ${actions.status.ok ? theme.success : theme.danger}`, borderRadius: 3, background: theme.panelAlt, color: actions.status.ok ? theme.text : theme.danger, fontSize: 11 }}>{actions.status.text}</div>}
