...
import type { TextClipPreset } from './textPresets';
...
interface LibraryPanelProps {
  ...
  onAddSolid?: () => void;
  onAddText: (preset: TextClipPreset) => void;
    ...
    <section className="cc-library-panel">
      <LibraryToolRail
        ...
        onSelect={(target) => {
          ...
          setMainTab(target.mainTab);
          if (target.subTab) setSubTab(target.subTab);
        }}
      />
      <div className="cc-library-content">
      {extensionOpen ? (
        <ExtensionCenter onClose={() => setExtensionOpen(false)} />
      ) : isText ? (
        <TextBrowser onAdd={onAddText} />
      ) : isCaptions ? (
      ...
      <>
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, padding: '10px 12px 7px 16px', fontSize: 12, borderBottom: `1px solid ${theme.border}` }}>
        <div style={{ display: 'flex', gap: 14, minWidth: 0, overflowX: 'auto', whiteSpace: 'nowrap', flex: 1 }}>
          {LIBRARY_RESOURCE_TABS.map((tab) => (
