...
try {
  const source = readFileSync(new URL('./mobileImport.ts', import.meta.url), 'utf8');
  assert.doesNotMatch(source, /^import .*heic-to/m, 'HEIC decoder must not enter the editor startup graph');
  assert.match(source, /await import\('heic-to\/csp'\)/, 'HEIC conversion loads its decoder on demand');
