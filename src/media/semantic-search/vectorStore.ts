...
const DATABASE_NAME = LEGACY_PERSISTENCE_IDS.semanticIndexDatabase;
...
function semanticPayload(response: unknown): unknown {
  return response && typeof response === 'object' && 'semanticVectors' in response
    ? (response as { semanticVectors: unknown }).semanticVectors
    : null;
}
...
export async function searchSemanticVectorsRemote(
  ...
  try {
    ...
    const payload = semanticPayload(response);
    if (!Array.isArray(payload)) {
      if (payload === null) remoteSemanticFailed = true;
      return null;
    }
    const hits = payload as Array<{
      assetId: string;
      sampleTime: number;
  ...
  return serializeScopeMutation(scopeId, async () => {
    if (remoteSemanticAvailable()) {
      try {
        ...
        if (semanticPayload(response) === null) remoteSemanticFailed = true;
      } catch {
        remoteSemanticFailed = true;
      }
    }
...
export async function clearSemanticVectors(scopeId: string): Promise<void> {
  return serializeScopeMutation(scopeId, async () => {
    if (remoteSemanticAvailable()) {
      try {
        const response = await requestProjectStore({ operation: 'semantic-vectors-clear', scopeId });
        if (semanticPayload(response) === null) remoteSemanticFailed = true;
      } catch {
        remoteSemanticFailed = true;
      }
