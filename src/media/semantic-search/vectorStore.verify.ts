  ...
  (globalThis as Record<string, unknown>).window = {
    yoloCutDesktop: {
      projectStore: async (request: unknown) => {
        ...
        return { semanticVectors: { ok: true } };
      },
    },
  };
}
...
async function main(): Promise<void> {
  ...
  // (An empty server result is a valid search with no hits.)
  (globalThis as Record<string, unknown>).window = {
    yoloCutDesktop: {
      projectStore: async (request: unknown) => {
        requests.push({ request });
  ...
  assert.deepEqual(remote, [], 'an empty remote index returns an empty hit list');
