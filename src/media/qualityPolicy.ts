...
// v2 changes the first-run behavior from "always original" to hardware-aware
// auto proxies. An explicit choice made in v2 remains authoritative.
const PREVIEW_SOURCE_KEY = 'cc.previewSource.v2';
...
export function shouldPreferMasterPreview(
  _mode: QualityMode = current,
  source: PreviewSourceMode = previewSource,
): boolean {
  if (source === 'original') return true;
  if (source === 'proxy') return false;
  // Auto is an editing-performance decision. Proxies never replace the master
  // and never lower export quality, including in master-quality mode.
  return false;
}
