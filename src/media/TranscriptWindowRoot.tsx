...
export function TranscriptWindowRoot() {
  ...
  useEffect(() => {
    const desktop = window.yoloCutDesktop;
  ...
  const transcriptWindowTitle = t('YoloCut 文字稿');
  useEffect(() => {
    // Chromium replaces BrowserWindow's native title with index.html's title.
    // Preserve a distinct automation target and include the active asset name.
    document.title = asset?.name ? `${transcriptWindowTitle} — ${asset.name}` : transcriptWindowTitle;
  }, [asset?.name, transcriptWindowTitle]);
  if (!asset) return null;
  return (
    <TranscriptViewerDialog
      asset={asset}
      entries={entries}
      onClose={() => { void window.yoloCutDesktop?.windowAction('close'); }}
