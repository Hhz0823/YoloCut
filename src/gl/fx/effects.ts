...
import zoomBlurFrag from './zoom-blur.frag?raw';
import prismFrag from './prism.frag?raw';
import rippleFrag from './ripple.frag?raw';
import nightVisionFrag from './night-vision.frag?raw';
import thermalFrag from './thermal.frag?raw';
import comicFrag from './comic.frag?raw';
import embossFrag from './emboss.frag?raw';
import oldFilmFrag from './old-film.frag?raw';
import liquidGlassFrag from './liquid-glass.frag?raw';
...
export const FX_EFFECTS: Record<string, FxDef> = {
  ...
  // Open Effects Pack: dependency-free GLSL distributed with YoloCut under AGPL-3.0.
  'builtin:fx-vhs': {
    id: 'builtin:fx-vhs',
    name: 'VHS 录像带',
    desc: '磁带抖动、色度漂移、扫描线、噪点与跟踪线，适合复古录像和故障风格。动画。',
    frag: vhsFrag,
    props: [
      { key: 'intensity', label: '强度', default: 0.75, min: 0, max: 1.5, step: 0.01 },
      { key: 'tracking', label: '磁带抖动', default: 0.65, min: 0, max: 1, step: 0.01 },
      { key: 'chromaShift', label: '色度偏移', default: 3, min: 0, max: 16, step: 0.25 },
      { key: 'scanlines', label: '扫描线', default: 0.45, min: 0, max: 1, step: 0.01 },
      { key: 'noise', label: '噪点', default: 0.22, min: 0, max: 1, step: 0.01 },
      { key: 'speed', label: '速度', default: 1, min: 0.1, max: 5, step: 0.1 },
    ],
  },
  'builtin:fx-zoom-blur': {
    id: 'builtin:fx-zoom-blur',
    name: '变焦模糊',
    desc: '从可调中心向外拉出镜头变焦拖影，适合速度感、音乐卡点和冲击镜头。',
    frag: zoomBlurFrag,
    props: [
      { key: 'center_x', label: '中心 X', default: 0.5, min: 0, max: 1, step: 0.01 },
      { key: 'center_y', label: '中心 Y', default: 0.5, min: 0, max: 1, step: 0.01 },
      { key: 'amount', label: '模糊量', default: 0.45, min: 0, max: 1, step: 0.01 },
      { key: 'intensity', label: '强度', default: 0.8, min: 0, max: 1, step: 0.01 },
    ],
  },
  'builtin:fx-prism': {
    id: 'builtin:fx-prism',
    name: '棱镜色散',
    desc: '沿指定方向分离红蓝通道，可让画面边缘呈现镜头棱镜和折射色差。',
    frag: prismFrag,
    props: [
      { key: 'amount', label: '色散', default: 8, min: 0, max: 48, step: 0.5 },
      { key: 'angle', label: '方向', default: 0, min: 0, max: 6.2832, step: 0.05 },
      { key: 'falloff', label: '边缘增强', default: 0.7, min: 0, max: 1, step: 0.01 },
      { key: 'intensity', label: '强度', default: 1, min: 0, max: 1, step: 0.01 },
    ],
  },
  'builtin:fx-ripple': {
    id: 'builtin:fx-ripple',
    name: '水波扭曲',
    desc: '从指定中心扩散的动态水波折射，可调振幅、频率、速度和衰减。动画。',
    frag: rippleFrag,
    props: [
      { key: 'center_x', label: '中心 X', default: 0.5, min: 0, max: 1, step: 0.01 },
      { key: 'center_y', label: '中心 Y', default: 0.5, min: 0, max: 1, step: 0.01 },
      { key: 'amplitude', label: '振幅', default: 12, min: 0, max: 60, step: 0.5 },
      { key: 'frequency', label: '频率', default: 28, min: 2, max: 90, step: 1 },
      { key: 'speed', label: '速度', default: 1.2, min: -5, max: 5, step: 0.1 },
      { key: 'decay', label: '衰减', default: 2.2, min: 0, max: 10, step: 0.1 },
    ],
  },
  'builtin:fx-night-vision': {
    id: 'builtin:fx-night-vision',
    name: '夜视仪',
    desc: '按亮度增强暗部并映射为夜视色，叠加动态噪点、扫描线和镜头暗角。动画。',
    frag: nightVisionFrag,
    props: [
      { key: 'gain', label: '增益', default: 1.8, min: 0.5, max: 5, step: 0.05 },
      { key: 'noise', label: '噪点', default: 0.28, min: 0, max: 1, step: 0.01 },
      { key: 'scanlines', label: '扫描线', default: 0.35, min: 0, max: 1, step: 0.01 },
      { key: 'vignette', label: '暗角', default: 0.75, min: 0, max: 1, step: 0.01 },
      { key: 'tint', label: '夜视色', kind: 'color', default: [0.18, 1, 0.32], uniform: 'u_tint' },
    ],
  },
  'builtin:fx-thermal': {
    id: 'builtin:fx-thermal',
    name: '热成像',
    desc: '把画面亮度映射为蓝、青、黄、白的热成像色谱，可调亮度与对比。',
    frag: thermalFrag,
    props: [
      { key: 'contrast', label: '对比', default: 1.45, min: 0.25, max: 4, step: 0.05 },
      { key: 'brightness', label: '亮度', default: 0, min: -0.5, max: 0.5, step: 0.01 },
      { key: 'intensity', label: '强度', default: 1, min: 0, max: 1, step: 0.01 },
    ],
  },
  'builtin:fx-comic': {
    id: 'builtin:fx-comic',
    name: '漫画描边',
    desc: '颜色分阶结合实时边缘墨线，生成可调色阶、饱和度和线条强度的漫画效果。',
    frag: comicFrag,
    props: [
      { key: 'levels', label: '色阶', default: 6, min: 2, max: 16, step: 1 },
      { key: 'edgeStrength', label: '边缘强度', default: 4, min: 0, max: 12, step: 0.1 },
      { key: 'edgeThreshold', label: '边缘阈值', default: 0.12, min: 0, max: 0.6, step: 0.01 },
      { key: 'saturation', label: '饱和度', default: 1.25, min: 0, max: 2.5, step: 0.05 },
      { key: 'intensity', label: '强度', default: 1, min: 0, max: 1, step: 0.01 },
    ],
  },
  'builtin:fx-emboss': {
    id: 'builtin:fx-emboss',
    name: '浮雕',
    desc: '根据指定光照方向提取明暗差，生成灰度浮雕或与原画面混合的立体纹理。',
    frag: embossFrag,
    props: [
      { key: 'strength', label: '强度', default: 2.2, min: 0, max: 8, step: 0.1 },
      { key: 'angle', label: '方向', default: 0.75, min: 0, max: 6.2832, step: 0.05 },
      { key: 'blend', label: '混合', default: 0.85, min: 0, max: 1, step: 0.01 },
    ],
  },
  'builtin:fx-old-film': {
    id: 'builtin:fx-old-film',
    name: '老电影划痕',
    desc: '动态颗粒、胶片划痕、曝光闪烁和可调棕褐染色，模拟老式放映机。动画。',
    frag: oldFilmFrag,
    props: [
      { key: 'grain', label: '颗粒', default: 0.28, min: 0, max: 1, step: 0.01 },
      { key: 'scratches', label: '划痕', default: 0.5, min: 0, max: 1, step: 0.01 },
      { key: 'flicker', label: '闪烁', default: 0.45, min: 0, max: 1, step: 0.01 },
      { key: 'sepia', label: '棕褐', default: 0.42, min: 0, max: 1, step: 0.01 },
      { key: 'speed', label: '速度', default: 1, min: 0.1, max: 5, step: 0.1 },
    ],
  },
  'builtin:fx-liquid-glass': {
    id: 'builtin:fx-liquid-glass',
    name: '液态玻璃折射',
    desc: '动态双向波纹驱动玻璃折射、轻微色差与焦散高光，适合梦幻转场段落和产品镜头。动画。',
    frag: liquidGlassFrag,
    props: [
      { key: 'refraction', label: '折射强度', default: 18, min: 0, max: 80, step: 1 },
      { key: 'scale', label: '波纹尺度', default: 8, min: 1, max: 24, step: 0.25 },
      { key: 'speed', label: '速度', default: 0.8, min: -4, max: 4, step: 0.1 },
      { key: 'chromatic', label: '色差', default: 3, min: 0, max: 20, step: 0.5 },
      { key: 'highlight', label: '高光', default: 0.22, min: 0, max: 1, step: 0.01 },
      { key: 'mix', label: '混合', default: 0.88, min: 0, max: 1, step: 0.01 },
    ],
  },
};
