...
function animatedHoverOverrides(id: string, p: number): FxOverrides {
  ...
  if (id.includes('fx-vhs')) return { intensity: 0.35 + p, tracking: 0.25 + p * 0.75, chromaShift: 1 + p * 10 };
  if (id.includes('zoom-blur')) return { amount: 0.1 + p * 0.8, intensity: 0.35 + p * 0.65 };
  if (id.includes('fx-prism')) return { amount: 2 + p * 30, angle: p * 6.28 };
  if (id.includes('fx-ripple')) return { amplitude: 4 + p * 32, frequency: 14 + p * 46 };
  if (id.includes('night-vision')) return { gain: 1.1 + p * 2.7, noise: 0.1 + p * 0.5 };
  if (id.includes('fx-thermal')) return { contrast: 0.7 + p * 2.2, intensity: 0.4 + p * 0.6 };
  if (id.includes('fx-comic')) return { levels: 3 + p * 9, edgeStrength: 2 + p * 7 };
  if (id.includes('fx-emboss')) return { strength: 0.8 + p * 5, angle: p * 6.28 };
  if (id.includes('old-film')) return { grain: 0.1 + p * 0.55, scratches: 0.15 + p * 0.75 };
  if (id.includes('liquid-glass')) return { refraction: 5 + p * 52, chromatic: 1 + p * 10 };
