...
const FORMAT = 'yolocut.design-style';
const VERSION = 1;
...
function normalizeRecipe(value: unknown): DesignStyleRecipe {
  ...
  if ((recipe.format !== FORMAT && recipe.format !== LEGACY_PORTABLE_FORMATS.designStyle)
    || recipe.version !== VERSION) throw new Error('配方文件版本不受支持');
