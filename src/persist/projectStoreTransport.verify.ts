...
try {
  ...
  // One React effect may abort while a StrictMode remount is already awaiting
  // the same bootstrap. Cancellation is caller-local and must not poison the
  // shared request used by the live remount.
  invalidateEditorBootstrapInfo();
  let releaseBootstrap!: (value: { mcpToken: string }) => void;
  globals.window.yoloCutDesktop!.editorCredentials = () => {
    credentialCalls += 1;
    return new Promise((resolve) => { releaseBootstrap = resolve; });
  };
  const abortController = new AbortController();
  const cancelledBootstrap = editorBootstrapInfo(abortController.signal);
  const liveBootstrap = editorBootstrapInfo();
  abortController.abort();
  releaseBootstrap({ mcpToken: 'mcp-token-after-remount' });
  await assert.rejects(cancelledBootstrap, (error: unknown) => (
    error instanceof DOMException && error.name === 'AbortError'
  ));
  assert.deepEqual(await liveBootstrap, { mcpToken: 'mcp-token-after-remount' });
  assert.equal(credentialCalls, 3, 'concurrent callers share one uncancelled bootstrap request');
