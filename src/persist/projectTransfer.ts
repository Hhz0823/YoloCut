...
// Historical v1/v2 package identifiers remain readable for cross-version transfer.
...
export const PROJECT_EXPORT_FORMAT = 'yolocut-project@1';
export const PROJECT_STREAM_FORMAT = 'yolocut-project@2';
export const PROJECT_MIME = 'application/x-yolocut-project';
...
export function parseProjectEnvelope(
  ...
  if (r.format !== PROJECT_EXPORT_FORMAT && r.format !== LEGACY_PORTABLE_FORMATS.projectJson) {
...
function streamManifest(
  ...
  if ((manifest.format !== PROJECT_STREAM_FORMAT
    && manifest.format !== LEGACY_PORTABLE_FORMATS.projectStream)
...
export async function importProjectPackage(
  ...
  if ([PROJECT_STREAM_FORMAT, LEGACY_PORTABLE_FORMATS.projectStream]
    .some((format) => prefix.startsWith(`{"format":"${format}"`))) {
