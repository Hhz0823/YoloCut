...
const migrated = parseDesignStyleRecipe(JSON.stringify({
  ...built,
  format: LEGACY_PORTABLE_FORMATS.designStyle,
}));
assert.equal(migrated.format, built.format, 'historical recipes normalize to the YoloCut format');
