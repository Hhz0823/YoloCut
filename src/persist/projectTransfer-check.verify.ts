...
// ── parseProjectEnvelope ────────────────────────────────────────────────
{
  ...
  const migratedLegacy = parseProjectEnvelope(JSON.stringify({
    ...good,
    format: LEGACY_PORTABLE_FORMATS.projectJson,
  }));
  assert.ok('envelope' in migratedLegacy, 'historical project JSON remains importable');
  if ('envelope' in migratedLegacy) assert.equal(migratedLegacy.envelope.format, PROJECT_EXPORT_FORMAT);
