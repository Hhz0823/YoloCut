...
function staticBrowserBlocker(
  ...
  const dimensions = browserScaledExportDimensions(options.state, options.resolution);
  const scaledWidth = options.state.width * dimensions.scale;
  const scaledHeight = options.state.height * dimensions.scale;
  const integerScale = Number.isInteger(scaledWidth) && Number.isInteger(scaledHeight);
  if (!integerScale) {
    return {
      status: 'unsupported',
      reason: '该分辨率需要兼容渲染以保持精确的偶数像素尺寸',
      issues: [`fractional WebCodecs scale: ${scaledWidth}x${scaledHeight}`],
    };
  }
