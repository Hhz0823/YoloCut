...
assert.match(xml, /backgroundFill settings are preserved as YoloCut metadata/);
assert.match(xml, /key="com\.yolocut\.backgroundFillStrength" value="73"/,
  'the exact percentage survives in portable custom metadata');
assert.equal((xml.match(/key="com\.yolocut\.backgroundFill"/g) ?? []).length, 1,
