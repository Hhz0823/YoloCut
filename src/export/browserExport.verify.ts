...
const fractionalScale = await renderTimelineInBrowser({
  state,
  codec: 'h264',
  resolution: '480p',
  fps: 30,
...
assert.equal(fractionalScale.status, 'unsupported');
assert.match(
  fractionalScale.status === 'unsupported' ? fractionalScale.reason : '',
  /精确的偶数像素尺寸/,
);
assert.equal(loaderCalls, 0, 'fractional Remotion scale must route directly to compatible rendering');
...
globalThis.fetch = (async (input, init) => {
  ...
  if (url.startsWith('/export/stage?')) {
    ...
    return Response.json({
      path: '/media/uploads/yolocut-export-stage-browser-lifecycle.mp4',
      sizeBytes: blob.size,
    });
  }
  if (url.startsWith('/export/stage/')) {
