...
// ── Issue #27: preserve immutable original-media identity beside the internal working copy ──
{
  ...
  const xml = timelineToFcpxml(state, { mediaDir: '/Users/me/.yolocut/media' });
  const assetOpen = xml.match(/<asset(?=[\s>])[^>]*>/)?.[0] ?? '';
  ...
  assert.ok(xml.includes('kind="proxy-media" src="file:///Users/me/.yolocut/media/8e45fd6f-8da8-4d6a-8a4f-339d6a8fd747.mp4"'));
  ...
  assert.ok(xml.includes('<pathurl>file:///Users/me/.yolocut/media/8e45fd6f-8da8-4d6a-8a4f-339d6a8fd747.mp4</pathurl>'),
  ...
  }, { mediaDir: '/Users/me/.yolocut/media' });
  ...
  const withoutPool = timelineToFcpxml({ ...state, assets: undefined }, { mediaDir: '/Users/me/.yolocut/media' });
  ...
  assert.ok(uncXml.includes('kind="proxy-media" src="file://server/YoloCut/media/8e45fd6f-8da8-4d6a-8a4f-339d6a8fd747.mp4"'), 'UNC 代理路径合法编码');
}
...
// ── Resolve variants retain existing differences ──
{
  ...
  assert.ok(resolveXml.includes('<event name="YoloCut Export (Resolve)">'), 'Resolve 事件名');
