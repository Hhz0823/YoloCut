...
function backgroundFillMetadataXml(item: TimelineItem): string {
  return `<metadata>
          <md key="com.yolocut.backgroundFill" value="1" editable="1" type="boolean"/>
          <md key="com.yolocut.backgroundFillStrength" value="${backgroundFillStrengthOf(item)}" editable="1" type="integer"/>
        </metadata>`;
}
 ...
 * YoloCut timeline (independent absolute frame bits for each track) to FCPX magnetic timeline (connected clips with lane)
...
export function timelineToFcpxml(
  ...
  const title = escapeXml((opts.title ?? '').trim() || 'YoloCut Timeline');
  ...
  const backgroundFillWarning = fcpxmlBackgroundFillCount(state) > 0
    ? xmlComment('WARNING: backgroundFill settings are preserved as YoloCut metadata, but this exporter does not synthesize a portable blurred layer; render a video master to preserve the exact appearance.')
    : '';
  ...
  const eventName = nle === 'fcp_xml_resolve' ? 'YoloCut Export (Resolve)' : 'YoloCut Export';
