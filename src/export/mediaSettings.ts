 ...
 * exact integers. Some short-edge presets (notably 1920x1080 -> 854x480)
 * cannot satisfy that with one uniform scale, so render at 1x and conform the
 * result to the requested dimensions in FFmpeg. */
export function remotionExportScale(
  state: { width?: unknown; height?: unknown },
  resolution?: ExportResolution,
): number {
  const width = canvasDimension(state.width, 1920);
  const height = canvasDimension(state.height, 1080);
  const scale = exportScale(state, resolution);
  return Number.isInteger(width * scale) && Number.isInteger(height * scale) ? scale : 1;
}
...
export function webScaledExportDimensions(
  ...
  const width = canvasDimension(state.width, 1920);
  const height = canvasDimension(state.height, 1080);
  const widthScale = plan.width / width;
  const exactWidth = width * widthScale;
  const exactHeight = height * widthScale;
  const scale = Number.isInteger(exactWidth)
    && Number.isInteger(exactHeight)
    && exactWidth === plan.width
    && exactHeight === plan.height
    ? widthScale
    : plan.browserScale;
  return { width: plan.width, height: plan.height, scale };
}
