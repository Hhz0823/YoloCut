...
import { PRODUCT } from '../../shared/product-brand';

export const UPSTREAM_LATEST_RELEASE_URL = `https://api.github.com/repos/${PRODUCT.repository.owner}/${PRODUCT.repository.name}/releases/latest`;
export const UPSTREAM_RELEASES_URL = `${PRODUCT.repository.url}/releases/latest`;
...
type DesktopUpdateApi = NonNullable<Window['yoloCutDesktop']>['updates'];
...
function desktopUpdateApi(): DesktopUpdateApi | null {
  ...
  return window.yoloCutDesktop?.updates ?? null;
}
