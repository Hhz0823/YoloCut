...
/** Find the active clip in a start-frame-sorted, non-overlapping run. */
export function timelineItemAtFrame(items: readonly TimelineItem[], frame: number): TimelineItem | undefined {
  let low = 0;
  ...
  return candidate && frame < candidate.startFrame + candidate.durationInFrames ? candidate : undefined;
}
...
export function mergeFrameRanges(ranges: readonly FrameRange[]): FrameRange[] {
  const sorted = ranges
    ...
    .map(([from, to]) => [from, to] as FrameRange)
...
export function frameInSortedRanges(ranges: readonly FrameRange[], frame: number): boolean {
  let low = 0;
  let high = ranges.length - 1;
  while (low <= high) {
    const middle = (low + high) >>> 1;
    const [from, to] = ranges[middle]!;
    if (frame < from) high = middle - 1;
    else if (frame >= to) low = middle + 1;
    else return true;
  }
  return false;
}
