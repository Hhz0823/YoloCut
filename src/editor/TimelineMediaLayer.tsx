...
import { timelineItemAtFrame } from './transitionAudio';
...
export function ContinuousVideoAudio({ items, muted, gainAt, premountFor, browserRenderer }: {
  ...
  const volume = (frame: number) => {
    ...
    const item = timelineItemAtFrame(items, timelineFrame);
...
interface SharedVideoVisualGroupProps {
  group: TimelineItem[];
  fit: AspectFit;
  muted: boolean;
  canvasW: number;
...
function SharedVideoVisualContent({ group, fit, muted, canvasW, canvasH, browserRenderer }: Omit<SharedVideoVisualGroupProps, 'premountFor'>) {
  const frame = useCurrentFrame();
  const first = group[0];
  if (!first || !first.src) return null;
  const timelineFrame = first.startFrame + frame;
  const item = timelineItemAtFrame(group, timelineFrame) ?? first;
  const localFrame = timelineFrame - item.startFrame;
...
export function SharedVideoVisualGroup(props: SharedVideoVisualGroupProps) {
  const first = props.group[0];
  const last = props.group.at(-1);
  if (!first || !last || !first.src) return null;
  const duration = last.startFrame + last.durationInFrames - first.startFrame;
  return (
    <Sequence from={first.startFrame} durationInFrames={duration} premountFor={props.premountFor} name={`${first.name}:visual`}>
      <SharedVideoVisualContent {...props} />
    </Sequence>
  );
}
