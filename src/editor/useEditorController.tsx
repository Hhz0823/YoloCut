...
import { useProjectWindowSync } from './projectWindowSync';
...
export interface EditorProps {
  ...
  /** Dedicated native Agent window; keeps its editor context synced to main. */
  agentWindow?: boolean;
}

const NO_AUTO_MUSIC_ASSETS: ProjectDoc['assets'] = [];

function useEditorBaseStage({ initial, project, agentWindow }: EditorProps) {
  const t = useT();
  const editor = useEditor(initial);
  useAutoMusicAnalysis(agentWindow ? NO_AUTO_MUSIC_ASSETS : editor.doc.assets);
...
function useEditorUiStage(props: EditorProps, base: BaseStage, environment: EnvironmentStage) {
  ...
  const dialogs = useEditorWorkspaceDialogs({
    projectId: props.project.id,
    doc,
    playerRef: agent.playerRef,
    automaticVersions: !props.agentWindow,
  });
  const persistence = useEditorProjectPersistence({
    ...
    resumeGenerationJobs: !props.agentWindow,
  });
  const panels = useEditorWorkspacePanels({ commands, projectId: props.project.id });
  ...
  const exportActions = useEditorWorkspaceExportActions({
    ...
    resumeBackgroundExports: !props.agentWindow,
  });
...
function buildEditorWorkspaceViewProps(
  ...
  const { chatCollapsed, setChatCollapsed, panelLayout, inspectorCollapsed, setInspectorCollapsed, addTemplate, agentWorkbench } = ui.panels;
  ...
  return {
    ...
    chatPanel: {
      ...
      onImportMedia: importToPool,
      onOpenSettings: () => setShowSettings(true),
      hostMode: 'editor',
      dockSide: agentWorkbench.dockSide,
      canDetach: agentWorkbench.canDetach,
      onDetach: agentWorkbench.detach,
      onDock: agentWorkbench.dock,
    },
    chatPlacement: agentWorkbench.placement,
    agentDockPreview: agentWorkbench.dockPreview,
    chatCollapsed,
    onResizeChat: panelLayout.resizeChat,
    libraryPanel: {
      semanticScopeId: project.id,
      templates: allTemplates,
      onAddTemplate: addTemplate,
      onAddAudio: (asset) => commands.addAudio(asset),
      playerRef,
      fps: state.fps,
      items: state.items,
      trackOptions,
      captionTracks,
      onSetCaptions: commands.setCaptions,
      onUpdateCaptions: commands.updateCaptions,
      onSetItemTranscript: commands.setItemTranscript,
      onToggleWord: commands.toggleWord,
      onCleanScript: commands.cleanScript,
      onSetGapCap: commands.setGapCap,
      onSetTranscriptPlayOrder: commands.setTranscriptPlayOrder,
      onReorderTrackItems: commands.reorderTrackItems,
      onClearEdits: commands.clearEdits,
      assets: state.assets ?? [],
      mediaFolders: doc.mediaFolders,
      usedAssetIds,
      offlineAssetIds,
      onAssetLoadError: (asset) => markMediaOffline(asset.src),
      onImportMedia: importToPool,
      onImportMobileMedia: importMobileUpload,
      onIngestDirectoryAsset: ingestToPool,
      onTranscribeAsset: startAssetTranscription,
      onAddMediaItem: (asset) => commands.addMediaItem(asset),
      onAddMediaAssetsToTimeline: addMediaAssetsToTimeline,
      onUseMediaAI: useMediaAI,
      onPasteMediaAssets: pasteMediaAssets,
      onCreateMediaFolder: commands.createMediaFolder,
      onRenameMediaFolder: commands.renameMediaFolder,
      onDeleteMediaFolder: commands.deleteMediaFolder,
      onMoveMediaAssets: commands.moveMediaAssets,
      onRenameMediaAsset: commands.renameMediaAsset,
      onRenameMediaAssets: commands.renameMediaAssets,
      onSetMediaAssetFavorite: commands.setMediaAssetFavorite,
      onSetMediaAssetsFavorite: commands.setMediaAssetsFavorite,
      onRemoveMediaAsset: commands.removeMediaAsset,
      onRemoveMediaAssets: commands.removeMediaAssets,
      ...
      onAddText: (preset) => {
        const id = commands.addTextClip({ startFrame: getPlayhead(), ...preset });
      ...
      selectedItem,
      onApplyTransition: (type, custom) => state.selectedId
        && commands.addTransition(state.selectedId, type, undefined, custom),
      onApplyFx: (assetId) => {
        if (!state.selectedId) return;
        const item = state.items.find((candidate) => candidate.id === state.selectedId);
        if (!item) return;
        const previous = item.effects ?? [];
        const next = [
          ...previous.filter((effect) => effect.assetId !== assetId),
...
export function useEditorController(props: EditorProps): EditorWorkspaceViewProps {
  ...
  const role = props.agentWindow ? 'agent' : 'main';
  useProjectWindowSync(
    props.project.id,
    base.doc,
    base.commands,
    role,
    Boolean(props.agentWindow || ui.panels.agentWorkbench.placement === 'detached'),
  );
