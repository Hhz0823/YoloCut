...
import { useAgentWorkbench, type AgentWorkbenchHost } from '../hooks/useAgentWorkbench';
...
export function useEditorWorkspaceDialogs({
  ...
  useAutomaticVersions(projectId, doc, automaticVersions);
...
export interface EditorWorkspacePanels {
  ...
  agentWorkbench: AgentWorkbenchHost;
}
...
export function useEditorWorkspacePanels({
  ...
  const agentWorkbench = useAgentWorkbench(projectId);
  const panelLayout = useEditorPanelLayout(chatCollapsed, agentWorkbench.placement);
...
interface EditorWorkspaceExportActionsInput {
  ...
  resumeBackgroundExports?: boolean;
}

function useExportJobs(projectId: string, t: typeof translate, resume: boolean): [ExportJobStore, number] {
...
export function useEditorWorkspaceExportActions(
  ...
  const [exportJobs, activeExportJobs] = useExportJobs(
    input.projectId,
    input.t,
    input.resumeBackgroundExports ?? true,
  );
