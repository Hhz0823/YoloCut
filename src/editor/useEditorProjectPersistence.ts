...
import { AGENT_WORKBENCH_DOCKING_SESSION_KEY } from '../../shared/agent-workbench';
...
function useBrowserSaveGuards(projectId: string, enqueuePendingSave: () => unknown): void {
  useEffect(() => {
    ...
    const blockUnfinishedSave = (event: BeforeUnloadEvent): void => {
      enqueuePendingSave();
      if (sessionStorage.getItem(AGENT_WORKBENCH_DOCKING_SESSION_KEY) === '1') return;
...
function useGenerationJobResume(
  ...
  useEffect(() => {
    ...
    return () => { alive = false; };
  }, [projectId, commands, stateRef, docRef, enabled]);
}
...
export function useEditorProjectPersistence({
  ...
  useGenerationJobResume(projectId, commands, stateRef, docRef, resumeGenerationJobs);
