...
export function ShortcutsDialog({ onClose }: ShortcutsDialogProps) {
  ...
  return (
    ...
    >
      ...
      background: theme.panel, border: `1px solid ${theme.borderLight}`, borderRadius: 6, boxShadow: `0 18px 48px ${themeAlpha.shadow(0.55)}`,
        }}
      >
          ...
          {SHORTCUT_GROUPS.map((g) => {
            ...
            <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 10px', borderRadius: 3, background: theme.panelAlt }}>
                        ...
                        <button
                          ...
                          title={t('点击改绑')}
                          style={{
                            fontSize: 11, fontFamily: 'SFMono-Regular, ui-monospace, SFMono-Regular, Menlo, monospace',
                        ...
                        {conflicting && pending && (
      <div style={{ position: 'absolute', right: 24, marginTop: 40, zIndex: 1, background: theme.panel, border: `1px solid ${theme.accent}`, borderRadius: 4, padding: 10, boxShadow: `0 8px 24px ${themeAlpha.shadow(0.67)}`, maxWidth: 300 }}>
                            ...
                            <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                              <button type="button" onClick={() => setPending(null)} style={{ fontSize: 11, background: 'none', border: `1px solid ${theme.border}`, borderRadius: 6, padding: '3px 8px', color: theme.text, cursor: 'pointer' }}>{t('取消')}</button>
                              <button type="button" onClick={() => { setBinding(pending.id, pending.keys); setPending(null); }} style={{ fontSize: 11, background: theme.panelAlt, border: `1px solid ${theme.borderLight}`, borderRadius: 8, padding: '4px 8px', color: theme.textStrong, cursor: 'pointer' }}>{t('仍要绑定')}</button>
                            </div>
                          </div>
                        )}
                      </div>
