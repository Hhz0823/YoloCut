...
export function GenerationActivity({ projectId, onResume }: GenerationActivityProps) {
  ...
  return (
    <>
      ...
      />
      {open && createPortal(
        <div className="cc-modal-overlay" role="presentation" onClick={() => setOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 1200, background: 'rgba(0,0,0,0.28)' }}>
          <section
            className="cc-modal"
            role="dialog"
            aria-modal="true"
            ...
            style={{ position: 'absolute', top: 42, right: 72, width: 'min(460px, calc(100vw - 24px))', maxHeight: 'min(620px, calc(100vh - 64px))', overflow: 'auto', padding: 14, border: `1px solid ${theme.border}`, borderRadius: 8, background: theme.panel, boxShadow: '0 18px 48px rgba(0,0,0,0.34)' }}
          >
            <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <div>
                <div style={{ color: theme.text, fontSize: 13, fontWeight: 650 }}>{t('生成任务')}</div>
                <div style={{ color: theme.textDim, fontSize: 11, marginTop: 2 }}>{t('刷新后仍可继续检查、下载或重跑')}</div>
              </div>
            ...
            {loading && !jobs.length ? (
              ...
              return <article key={job.operationId} style={{ padding: '10px 0', borderTop: `1px solid ${theme.border}` }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px 10px', color: theme.textDim, fontSize: 10.5, marginTop: 6 }}>
                  <code title={job.operationId} style={{ fontFamily: 'SFMono-Regular, ui-monospace, SFMono-Regular, monospace' }}>{job.operationId}</code>
...
const ghostButton: React.CSSProperties = { border: `1px solid ${theme.border}`, borderRadius: 5, background: theme.panelAlt, color: theme.text, cursor: 'pointer', padding: '4px 9px', fontSize: 11.5 };
