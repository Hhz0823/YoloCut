...
import { LOCAL_VOICE_SETTINGS_FIELDS } from './localVoiceSettings';
...
const CODEX_PAGE: SettingsVendorPage = {
  ...
  title: 'OpenAI · Codex',
  connection: 'codex',
  note: '使用 ChatGPT 订阅登录，由官方 Codex CLI 管理凭据、续期与退出。YoloCut 不会读取或显示 OAuth 凭据。',
  fields: [
    {
      name: 'CODEX_MODEL',
      label: 'Codex 模型',
...
export const SETTINGS_CATEGORIES: readonly SettingsCategory[] = [
      ...
      { key: 'storage', title: '媒体存储', hint: '工程与素材的本地保存目录，与可选的 R2 云备份。',
        vendors: [
          { key: 'storage/projects', vendor: 'localdisk', title: '工程存储目录',
            ...
            fields: [
              directory('YOLOCUT_DATA_DIR', '工程存储目录', '应用默认数据目录',
  ...
  },
  {
    ...
    groups: [
      { key: 'local', title: '本地模型', hint: '本地转写、口播、节拍与语义分析。模型按需安装，数据不出本机。',
        vendors: [
          { key: 'local/asr', vendor: 'localasr', title: '本地转写', icon: 'mic', kind: 'local-models', fields: localAsrPage.fields },
          { key: 'local/voice', vendor: 'localasr', title: '本地口播', icon: 'volume', kind: 'local-models', fields: LOCAL_VOICE_SETTINGS_FIELDS },
          ...
          { key: 'local/auto-edit', vendor: 'localasr', title: '自动剪辑分析', icon: 'sparkles', kind: 'local-models', fields: [] },
        ] },
    ],
  },
];
