...
function useFieldContext(
  ...
  reveal: boolean,
  onServerSettingsApplied: FieldCtx['onServerSettingsApplied'],
): FieldCtx {
  ...
  const onModelsDiscovered = useCallback((name: string, models: readonly string[]) => {
    setModelOptions((previous) => ({ ...previous, [name]: [...new Set(models)] }));
  }, []);
  return {
    status, values, reveal, onStage, onToggleClear, modelOptions, codex, onServerSettingsApplied,
    onModelsDiscovered,
  };
}
...
export function SettingsDialog({ onClose }: { onClose: () => void }) {
  ...
  const ctx = useFieldContext(status, values, setValues, reveal, (next, clearedFields) => {
    setStatus(next);
    applySavedToAgent(next);
    setValues((previous) => clearedFields.reduce((current, name) => omitKey(current, name), previous));
  });
  useEffect(() => {
  ...
  const { save, saving, msg, error } = useSaveKeys(values, (next) => {
    ...
    void window.yoloCutDesktop?.windowAction('apply-ui-scale');
    applySavedToAgent(next);
  ...
  return (
    <div className="cc-settings-overlay" style={overlay} onMouseDown={requestClose}>
      <div className="cc-settings-dialog" role="dialog" aria-modal="true" aria-label={t('设置 · API 密钥')} style={panel} onMouseDown={(e) => e.stopPropagation()}>
        <header className="cc-settings-header cc-liquid-glass-host" style={head}>
          <LiquidGlassBackdrop />
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ color: theme.accent, display: 'inline-flex' }}><Icon name="sliders" size={15} /></span>
            <b className="cc-dialog-title cc-glass-ink" style={{ fontSize: 14 }}>{t('设置 · API 密钥')}</b>
          </div>
...
function CapabilityTree({ status, codexStatus, activeGroup, onSelect }: {
  ...
  return (
    <nav className="cc-settings-sidebar" style={sidebar}>
...
function GroupRow({ title, on, active, onSelect }: {
  ...
  return (
    <button type="button" onClick={onSelect} {...hoverProps}
      className={`cc-settings-nav-row${active ? ' active' : ''}`}
...
function VendorList({ group, activeVendor, onSelectVendor, ctx }: {
  ...
  return (
    <div className="cc-settings-vendors" style={vendorCol}>
...
function VendorRow({ page, on, active, onSelect }: {
  ...
  const [hovered, hoverProps] = useHover();
  return (
    <button type="button" className={`cc-settings-nav-row${active ? ' active' : ''}`} onClick={onSelect} {...hoverProps} style={navRowStyle(active, hovered)}>
...
function FooterBar({ reveal, onReveal, message, dirty, saving, onClose, onSave }: FooterBarProps) {
  ...
  return (
    <footer className="cc-settings-footer" style={foot}>
...
function navRowStyle(active: boolean, hovered: boolean): React.CSSProperties {
  return {
    ...
    width: '100%', padding: '7px 9px', borderRadius: 8, cursor: 'pointer', textAlign: 'left',
    border: `1px solid ${active ? theme.borderLight : 'transparent'}`,
...
const overlay: React.CSSProperties = {
  ...
  zIndex: 200, padding: 24, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", system-ui, sans-serif',
};
const panel: React.CSSProperties = {
  width: 'min(940px, 100%)', height: 'min(640px, 86vh)', display: 'flex', flexDirection: 'column',
    background: theme.panel, color: theme.text, border: `1px solid ${theme.borderLight}`, borderRadius: 12,
...
const iconBtn: React.CSSProperties = { background: 'none', border: 'none', color: theme.textDim, cursor: 'pointer', padding: 4, borderRadius: 5, display: 'inline-flex' };
  const btnGhost: React.CSSProperties = { font: 'inherit', fontSize: 12.5, background: 'transparent', color: theme.text, border: `1px solid ${theme.border}`, borderRadius: 8, padding: '7px 13px', cursor: 'pointer' };
  const btnPrimary: React.CSSProperties = { font: 'inherit', fontSize: 12.5, fontWeight: 600, background: theme.panelAlt, color: theme.textStrong, border: `1px solid ${theme.borderLight}`, borderRadius: 8, padding: '7px 16px' };
