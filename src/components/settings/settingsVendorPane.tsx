...
import { LocalVoiceModelPane } from './LocalVoiceModelPane';
    ...
    <div style={pane}>
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <b className="cc-settings-pane-title" style={{ fontSize: 13 }}>{t(page.title)}</b>
      ...
      {page.key === 'llm/zcode' && (
        <ZCodeConnectionCard
          onSettingsApplied={ctx.onServerSettingsApplied}
          onModelsDiscovered={ctx.onModelsDiscovered}
        />
      )}
...
function LocalModelsPane({ page, fields, ctx }: {
  ...
  const t = useT();
  const title = page.key === 'local/asr'
    ? '本地转写'
    : page.key === 'local/voice'
      ? '本地口播'
      : page.key === 'local/music/packs'
        ? '节拍与音乐分析'
        : page.key === 'local/semantic/setup'
          ? '画面语义搜索'
          : '自动剪辑分析';
  const description = page.key === 'local/voice'
    ? '本地口播模型按需安装；模型、音色、语言与硬件信息由后端目录提供。'
    : '本地模型按需安装，索引、转写和分析都在本机完成。';
  return (
    <div style={pane} className="cc-settings-pane">
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      {page.key === 'local/auto-edit' && <LocalModelPackPane
        packIds={['smolvlm2-500m-q8-local']}
        title="开源自动剪辑分析模型"
        description="SmolVLM2 仅提取参考成片的结构、节奏、字幕、转场与调色规律。模型不会自动安装；运行时需要官方 llama.cpp。"
        renderExtraMetadata={(pack) => pack.runtimeAvailability?.available === false
          ? <div style={{ marginTop: 5, fontSize: 11, color: theme.gold }}>{t(pack.runtimeAvailability.reason ?? '')}</div>
          : null} />}
    </div>
  );
}
  ...
  return (
    ...
    >
      <span aria-hidden style={{
        width: 30, height: 17, borderRadius: 8, position: 'relative', flexShrink: 0,
...
function DirectoryInput({ field, shown, stagedClear, onStage }: {
  ...
  const picker = window.yoloCutDesktop?.selectDirectory;
    ...
    <>
      <div style={{ display: 'flex', alignItems: 'stretch', gap: 7 }}>
        <input type="text" autoComplete="off" spellCheck={false} value={shown}
          ...
          style={{ ...(stagedClear ? { ...input, border: `1px solid ${WARN}` } : input), minWidth: 0 }} />
...
const pane: React.CSSProperties = {
  ...
  const fieldCardBox: React.CSSProperties = { background: theme.bg, border: `1px solid ${theme.border}`, borderRadius: 4, padding: '11px 13px' };
...
const browseBtn: React.CSSProperties = {
  ...
  border: `1px solid ${theme.border}`, borderRadius: 6, padding: '6px 11px',
...
const testBtn: React.CSSProperties = {
  ...
  border: `1px solid ${theme.border}`, borderRadius: 4, padding: '4px 11px', flex: '0 0 auto',
};
