...
assert.match(component, /createPortal\(/, '生成任务浮层应脱离带毛玻璃滤镜的顶部栏坐标系');
assert.match(component, /document\.body/, '生成任务浮层应挂载到 document.body');
assert.match(component, /event\.key === 'Escape'/, '生成任务浮层应支持 Escape 关闭');
assert.match(component, /role="dialog"/, '生成任务浮层应暴露对话框语义');
