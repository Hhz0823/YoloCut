...
export function LocaleToggle() {
  ...
  return (
    <button
      ...
      style={{ minWidth: 30, height: 22, background: 'none', border: `1px solid ${theme.border}`, borderRadius: 4, cursor: 'pointer', padding: '0 5px', fontSize: 11, fontWeight: 600, letterSpacing: 0.3, color: theme.textDim, display: 'grid', placeItems: 'center' }}
...
export function TopBar({ projectId, projectName, canUndo, canRedo, exporting, exportJobCount = 0, onHome, onRename, onResumeGeneration }: TopBarProps) {
  ...
  return (
    <header className={`cc-topbar cc-liquid-glass-host cc-window-titlebar${isMacDesktop ? ' cc-window-titlebar--mac' : ''}`} style={{ gridColumn: '1 / -1', gridRow: 1, position: 'relative', height: '100%', display: 'flex', alignItems: 'center', padding: '0 12px', borderBottom: `1px solid ${theme.border}`, background: theme.bg, gap: 6 }}>
      <LiquidGlassBackdrop />
      ...
      {/* OpenCut-style left-aligned project title keeps the editing actions stable. */}
      <div className="cc-topbar-title" style={{ display: 'flex', gap: 8, justifyContent: 'flex-start', alignItems: 'center', fontSize: 13, fontWeight: 600, color: theme.text }}>
        {editing ? (
          <input autoFocus value={draft} onChange={(e) => setDraft(e.target.value)} onBlur={commit}
            onKeyDown={(e) => { if (e.key === 'Enter') commit(); if (e.key === 'Escape') setEditing(false); }}
            style={{ font: 'inherit', fontSize: 13, textAlign: 'left', background: theme.panelAlt, color: theme.text, border: `1px solid ${theme.accent}`, borderRadius: 5, padding: '4px 8px', minWidth: 200 }} />
        ) : (
          <span className="cc-glass-ink" onDoubleClick={() => { if (onRename) { setDraft(projectName); setEditing(true); } }} title={onRename ? t('双击重命名') : undefined} style={{ cursor: onRename ? 'text' : 'default' }}>{projectName}</span>
        )}
      </div>
      ...
      <button onClick={() => invokeAction('open-export', undefined, 'toolbar')}
        className="cc-primary-action cc-tip cc-tip-r"
        data-tip={exporting ? t('查看后台导出任务') : t('导出 MP4')}
        aria-label={exporting ? t('查看后台导出任务') : t('导出 MP4')}
        style={{ minWidth: 64, height: 30, background: theme.panelAlt, color: theme.textStrong, border: `1px solid ${theme.borderLight}`, borderRadius: 8, padding: '0 11px', fontSize: 12, fontWeight: 650, cursor: 'pointer', marginLeft: 6 }}>
        {exporting ? t('{n} 个导出', { n: Math.max(1, exportJobCount) }) : t('导出')}
      </button>
      <div className="cc-account-placeholder" title={t('账户')}><Icon name="users" size={13} /></div>
      </div>
