...
export type IconName =
  ...
  | 'zoomOut' | 'zoomIn' | 'fit' | 'hand' | 'aspect' | 'captions' | 'fullscreen'
...
const P: Record<IconName, string> = {
  ...
  hand: 'M7 11V7a1.5 1.5 0 0 1 3 0v3-5a1.5 1.5 0 0 1 3 0v4-3a1.5 1.5 0 0 1 3 0v4-1a1.5 1.5 0 0 1 3 0v6c0 4-3 7-7 7h-1a7 7 0 0 1-5.7-2.9L3.5 15a1.7 1.7 0 0 1 2.7-2z',
...
export function BrandMark({ size = 16 }: { size?: number }) {
  return (
    <img src="/yolocut-icon.png" alt="" aria-hidden width={size} height={size} style={{ display: 'block' }} />
  );
}

/** YoloCut word mark. The icon remains the protocol-compatible inherited asset. */
export function YoloCutWordmark({ width = 126, className }: { width?: number; className?: string }) {
  const gradientId = `cc-wordmark-${useId().replace(/:/g, '')}`;
  return (
    <svg
      className={`cc-wordmark${className ? ` ${className}` : ''}`}
      aria-label="YoloCut"
      role="img"
      width={width}
      height={width / 4}
    ...
    >
      ...
      >
        <tspan className="cc-wordmark-solid" x="6" y="87" fill="currentColor" fontSize="76" fontWeight="600" letterSpacing="-0.035em">
          YoloCut
        </tspan>
        <tspan className="cc-wordmark-mixed" x="6" y="87" fill={`url(#${gradientId})`} fontSize="76" fontWeight="600" letterSpacing="-0.035em">
          YoloCut
        </tspan>
      </text>
    </svg>
