...
export function TimelineToolbar({
  ...
  const [customCanvasOpen, setCustomCanvasOpen] = useState(false);
  ...
  const activeAspectPreset = ASPECT_PRESETS.find((preset) => preset.width === state.width && preset.height === state.height);
  return (
    <>
      ...
      <div className="cc-timeline-tool-group">
        ...
        {recorder.recording && <span title={t('录音中')} style={{ width: 8, height: 8, borderRadius: 4, background: theme.accent, flexShrink: 0 }} />}
      ...
      <label className="cc-aspect-select cc-tip cc-tip-r" data-tip={t('画幅比例')}>
        ...
        <select aria-label={t('画幅比例')} value={activeAspectPreset?.label ?? '__current_custom__'}
          onChange={(event) => {
            if (event.target.value === '__custom__') {
              setCustomCanvasOpen(true);
              return;
            }
          ...
          <optgroup label={t('画幅比例')}>
            {ASPECT_PRESETS.map((preset) => <option key={preset.label} value={preset.label}>{preset.label}</option>)}
            {!activeAspectPreset && <option value="__current_custom__">{state.width} × {state.height}</option>}
            <option value="__custom__">{t('自定义尺寸…')}</option>
          </optgroup>
      ...
      {customCanvasOpen && (
        <CustomCanvasSizeDialog
          width={state.width}
          height={state.height}
          onClose={() => setCustomCanvasOpen(false)}
          onApply={(width, height) => {
            commands.setAspect(width, height, state.fit);
            setCustomCanvasOpen(false);
          }}
        />
      )}
    </>
