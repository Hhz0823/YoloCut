...
export function MarkerEditor({ editing, fps, commands, onClose }: MarkerEditorProps) {
  const t = useT();
  return (
    <div style={{ position: 'absolute', top: 40, left: 12, zIndex: 20, width: 260, background: theme.panelAlt, border: `1px solid ${theme.border}`, borderRadius: 5, padding: 12, boxShadow: `0 8px 28px ${themeAlpha.shadow(0.45)}` }}>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        ...
        <span style={{ flex: 1 }} />
        <button onClick={onClose} style={{ background: theme.panelAlt, border: `1px solid ${theme.borderLight}`, borderRadius: 8, color: theme.textStrong, cursor: 'pointer', fontSize: 12, padding: '5px 12px' }}>{t('完成')}</button>
      </div>
    </div>
  );
}
