...
import { editableBezierControlPoints } from './bezierCurve';
      ...
      {at && (
        <>
          <select
            ...
            title={next ? t('缓动（此关键帧到下一帧的曲线）') : t('最后一个关键帧没有后续曲线')}
            disabled={!next}
            onChange={(e) => {
              const v = e.target.value;
              if (v === 'bezier') {
                onSet(localFrame, at.value, editableBezierControlPoints(at.easing));
                setCurveOpen(true);
                return;
              }
          ...
          >
            ...
            <option value="bezier">{t('贝塞尔')}</option>
          </select>
          <button
            className="cc-insp-kf-btn curve"
            type="button"
            disabled={!next}
            title={next ? t('打开关键帧曲线编辑器') : t('最后一个关键帧没有后续曲线')}
            aria-label={next ? t('打开关键帧曲线编辑器') : t('最后一个关键帧没有后续曲线')}
          ...
          {curveOpen && next && (
            <BezierCurveEditor
              easing={at.easing}
              onChange={(easing) => onSet(localFrame, at.value, easing)}
