...
export function ExportHistory() {
  ...
  return (
    <>
      ...
      {open && createPortal(
        <div className="cc-modal-overlay" role="presentation" onClick={() => setOpen(false)} style={backdrop}>
          <section
            className="cc-modal"
            role="dialog"
            aria-modal="true"
            aria-labelledby="cc-export-history-title"
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '13px 16px', borderBottom: `1px solid ${theme.border}` }}>
              ...
              <span id="cc-export-history-title" style={{ flex: 1, fontSize: 14, fontWeight: 600 }}>{t('导出历史')}</span>
              <button type="button" onClick={() => setOpen(false)} aria-label={t('关闭')} title={t('关闭')} style={iconBtn}><Icon name="x" size={15} /></button>
            </div>
              ...
              {loading ? (
                ...
                records.map((r) => (
                  <div key={r.id} style={row}>
                    ...
                    {r.destinationId && window.yoloCutDesktop?.revealExport && (
                      <button type="button" onClick={() => {
                        void window.yoloCutDesktop?.revealExport(r.destinationId!, r.name).catch(() => undefined);
                      }}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '12px 16px', borderTop: `1px solid ${theme.border}` }}>
...
const card: React.CSSProperties = {
  width: 420, maxWidth: '100%', maxHeight: '80vh', display: 'flex', flexDirection: 'column',
    background: theme.panel, color: theme.text, border: `1px solid ${theme.border}`, borderRadius: 6,
  boxShadow: `0 20px 60px ${themeAlpha.shadow(0.5)}`,
};
const row: React.CSSProperties = {
  display: 'flex', alignItems: 'center', gap: 9, background: theme.panelAlt,
    border: `1px solid ${theme.border}`, borderRadius: 4, padding: '7px 10px',
};
...
const folderBtn: React.CSSProperties = {
  ...
  padding: '0 9px', border: `1px solid ${theme.border}`, borderRadius: 4,
