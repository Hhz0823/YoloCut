  ...
  return (
    <div style={{ margin: '9px 0' }}>
      ...
      >
        ...
        <span style={{ fontFamily: 'SFMono-Regular, ui-monospace, SFMono-Regular, Menlo, monospace', letterSpacing: 0.2 }}>{name}</span>
        <span style={{ opacity: 0.8 }}>· {t('{n} 次', { n: items.length })}</span>
        <span style={{ fontSize: 9, opacity: 0.7 }}>{open ? '▾' : '▸'}</span>
      </button>
      {open && (
        <div style={{ marginLeft: 3, paddingLeft: 9, borderLeft: `1px solid ${theme.border}` }}>
          {items.map(({ msg, index }) => (
