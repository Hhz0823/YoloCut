...
assert.match(pendingComposerMarkup, /id="cc-agent-composer-input"/, 'textarea needs a stable desktop automation id');
assert.match(pendingComposerMarkup, /name="yolocut-agent-prompt"/, 'textarea needs a stable form/automation name');
assert.match(pendingComposerMarkup, /data-cc-agent-input="true"/, 'textarea needs a dedicated Agent selector');
assert.match(pendingComposerMarkup, /aria-label="Agent 提示词输入"/, 'textarea needs a stable accessible name independent of placeholder state');
