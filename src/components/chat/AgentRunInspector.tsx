...
const mono: CSSProperties = { fontFamily: 'SFMono-Regular, ui-monospace, SFMono-Regular, Menlo, monospace' };
...
export function AgentRunInspector({ projectId }: { projectId: string }) {
  ...
  return <>
    <button ref={triggerRef} type="button" aria-haspopup="dialog" aria-expanded={open} aria-controls="cc-agent-run-inspector"
      ...
      data-cc-agent-no-drag className="cc-header-btn cc-agent-header-action" style={trigger}>
...
const popover: CSSProperties = { position: 'fixed', display: 'flex', flexDirection: 'column', overflow: 'hidden', color: theme.text, border: `1px solid ${theme.borderLight}`, borderRadius: 7, background: theme.panelAlt, boxShadow: `0 14px 42px ${themeAlpha.shadow(0.5)}` };
const header: CSSProperties = { display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, padding: '11px 12px', borderBottom: `1px solid ${theme.border}` };
...
const interrupted: CSSProperties = { margin: '0 12px 8px', padding: 8, border: `1px solid ${theme.gold}`, borderRadius: 4, color: theme.text, background: themeAlpha.ink(0.04), fontSize: 11, lineHeight: 1.45 };
...
const serverReason: CSSProperties = { margin: '0 12px 8px', padding: 8, border: `1px solid ${theme.border}`, borderRadius: 4, color: theme.text, background: themeAlpha.ink(0.03), fontSize: 11, lineHeight: 1.45 };
const section: CSSProperties = { padding: '9px 12px', borderTop: `1px solid ${theme.border}` };
