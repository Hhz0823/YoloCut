...
import type { AgentDockSide, AgentWorkbenchDetachPoint } from '../../../shared/agent-workbench';
...
export interface ChatPanelProps {
  ...
  onOpenSettings?: () => void;
  hostMode?: 'editor' | 'detached';
  dockSide?: AgentDockSide;
  canDetach?: boolean;
  onDetach?: (point?: AgentWorkbenchDetachPoint) => void | Promise<void>;
  onDock?: (side: AgentDockSide) => void | Promise<void>;
}
...
export interface ChatPanelActions {
  ...
  importRoleFiles: (role: AutoEditAttachmentRole, files: File[]) => Promise<void>;
...
function useComposerProject(composer: ChatComposerController, projectId: string): void {
  ...
  useEffect(() => () => saveComposerDraft(projectId, liveInputRef.current), [projectId]);
...
function useChatActions(
  ...
  const importRoleFiles = (role: AutoEditAttachmentRole, files: File[]) => importPastedFiles(files, { role });
  return { ...messages, ...references, importPastedFiles, importRoleFiles };
}
