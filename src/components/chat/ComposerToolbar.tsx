    ...
    <div className="cc-chat-composer-bar">
      ...
      {running ? (
        <button title={t('停止')} onClick={onStop} className="cc-chat-send-btn"
          style={{ width: 28, height: 28, borderRadius: 8, border: `1px solid ${theme.borderLight}`, background: theme.panelAlt, cursor: 'pointer', display: 'grid', placeItems: 'center', flexShrink: 0 }}>
          <span style={{ width: 10, height: 10, background: theme.danger, borderRadius: 2 }} />
        </button>
      ) : (
        <button title={sendTitle} onClick={onSubmit} disabled={!canSend} className="cc-chat-send-btn"
          style={{ width: 28, height: 28, borderRadius: 8, border: `1px solid ${theme.borderLight}`, background: theme.panelAlt, color: canSend ? theme.accent : theme.textDim, cursor: canSend ? 'pointer' : 'default', display: 'grid', placeItems: 'center', lineHeight: 0, flexShrink: 0 }}>
