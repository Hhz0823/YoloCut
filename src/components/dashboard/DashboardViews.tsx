...
const LazyDashboardDialogs = lazy(() => import('./DashboardDialogs'));

function ModelSetupCard({ onOpen }: { onOpen: () => void }) {
  const t = useT();
  return (
    <section className="cc-dashboard-model-card" role="status" style={modelSetupCard}>
      ...
      <button className="cc-primary-action" type="button" onClick={onOpen} style={modelSetupButton}>{t('配置模型')}</button>
    </section>
  );
}
...
export function DashboardTitlebarContent({ model }: { model: DashboardModel }) {
  ...
  return (
    <>
      <BrandMark size={20} />
      <YoloCutWordmark className="cc-glass-ink" />
      <span className="cc-dashboard-subtitle cc-glass-muted-ink" style={{ color: theme.textDim, fontSize: 13 }}>{t('· 我的工程')}</span>
      <span className="cc-dashboard-actions" style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: 2 }}>
        <DashboardHeaderLinks />
        <button onClick={() => model.setDialog('mcp', true)} data-tip={t('外部 Agent 接入 (MCP)')} aria-label={t('外部 Agent 接入 (MCP)')} className="cc-dashboard-mcp cc-header-btn cc-tip cc-tip-r" style={settingsBtn}><Icon name="plug" size={16} /></button>
        <button onClick={() => model.setDialog('shortcuts', true)} data-tip={t('编辑快捷键')} aria-label={t('编辑快捷键')} className="cc-dashboard-shortcuts cc-header-btn cc-tip cc-tip-r" style={settingsBtn}><Icon name="keyboard" size={16} /></button>
        <span className="cc-dashboard-locale"><LocaleToggle /></span>
        <span className="cc-dashboard-skin"><SkinPicker /></span>
        <button onClick={() => model.setDialog('storage', true)} data-tip={t('数据存储')} aria-label={t('数据存储')} className="cc-dashboard-storage cc-header-btn cc-tip cc-tip-r" style={settingsBtn}><Icon name="database" size={16} /></button>
        <button onClick={() => model.setDialog('settings', true)} data-tip={t('设置 · API 密钥')} aria-label={t('设置 · API 密钥')} className="cc-dashboard-settings cc-header-btn cc-tip cc-tip-r" style={settingsBtn}><Icon name="sliders" size={16} /></button>
      </span>
    </>
  );
}
  ...
  return (
    <div className="cc-dashboard-toolbar" style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
      ...
      <span style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: 10 }}>
        ...
        <button className="cc-secondary-action" onClick={() => model.setDialog('cleanup', true)} style={importBtn} title={t('清理所有工程都不引用的上传素材(测试/已删工程残留)')}><Icon name="trash" size={13} /> {t('清理素材')}</button>
        <button className="cc-secondary-action" onClick={() => model.transfer.fileRef.current?.click()} disabled={model.transfer.busy} style={importBtn} title={t('导入 .ccproj 工程文件(兼容旧 .ccproj.json)')}><Icon name="upload" size={13} /> {t('导入工程')}</button>
        <input ref={model.transfer.fileRef} type="file" accept={`.ccproj,.json,application/json,application/x-yolocut-project,${LEGACY_PORTABLE_FORMATS.projectMime}`} onChange={model.transfer.pickImport} style={{ display: 'none' }} />
...
function ProjectCard({ project, props, model }: { project: ProjectMeta; props: DashboardProps; model: DashboardModel }) {
  const t = useT();
  return (
    <article className="cc-project-card" style={card}>
      <button className="cc-project-thumbnail" onClick={() => props.onOpen(project.id)} style={thumb} title={t('打开 {name}', { name: project.name })}>
  ...
  return (
    <>
      <div className="cc-dashboard-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(232px, 1fr))', alignItems: 'start', gap: 16 }}>
        <button className="cc-new-project-card" onClick={props.onNew} style={newCard} title={t('新建工程')}>
...
export function DashboardContent({ props, model }: { props: DashboardProps; model: DashboardModel }) {
  return (
    <main className="cc-dashboard-main" style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
      <div className="cc-dashboard-container" style={{ maxWidth: 1120, margin: '0 auto', padding: '28px 24px 80px' }}>
...
export function DashboardDialogs({ model }: { model: DashboardModel }) {
  const open = Object.values(model.dialogs).some(Boolean);
  if (!open) return null;
  return (
    <Suspense fallback={<DashboardDialogLoading />}>
      <LazyDashboardDialogs model={model} />
    </Suspense>
  );
}

function DashboardDialogLoading() {
  const t = useT();
  return (
    <div className="cc-settings-overlay" style={{ position: 'fixed', inset: 0, zIndex: 120, display: 'grid', placeItems: 'center', background: 'rgba(28,28,30,.72)', backdropFilter: 'blur(12px)' }}>
      <div className="cc-settings-dialog cc-dialog-loading" role="status" aria-live="polite" style={{ minWidth: 220, padding: 18, border: '1px solid rgba(255,255,255,.1)', borderRadius: 12, background: theme.panel, color: theme.textDim, textAlign: 'center' }}>
        {t('加载中…')}
      </div>
    </div>
  );
