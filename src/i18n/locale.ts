...
function syncLocaleSurfaces(locale: Locale): void {
  if (typeof document !== 'undefined') {
    document.documentElement.lang = locale === 'en' ? 'en' : 'zh-CN';
  }
  if (typeof window === 'undefined') return;
  const setDesktopLocale = window.yoloCutDesktop?.setLocale;
  if (typeof setDesktopLocale !== 'function') return;
  void setDesktopLocale(locale).catch(() => undefined);
}

syncLocaleSurfaces(current);
