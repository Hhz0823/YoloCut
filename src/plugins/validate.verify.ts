...
// ── 良性包全过 ────────────────────────────────────────────────────────────────
{
  ...
  const migrated = validatePack({ ...goodPack, format: LEGACY_PORTABLE_FORMATS.plugin });
  assert.ok(migrated.ok, 'historical v1 plugin packages remain installable');
  if (migrated.ok) assert.equal(migrated.pack.format, PLUGIN_FORMAT);
}
...
// ── propSchema 校验 + sha256Hex + 反注册(纯注册表侧) ─────────────────────────
{
  ...
  const hex = await sha256Hex('yolocut');
  assert.match(hex, /^[0-9a-f]{64}$/, 'sha256Hex 输出 64 位小写 hex');
  assert.equal(hex, await sha256Hex('yolocut'), '同文本稳定');
