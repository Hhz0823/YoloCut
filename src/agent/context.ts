...
export interface AssetReference {
  ...
  name: string;
  kind: AssetRefKind;
  metadata?: { readonly attachmentRole?: AutoEditAttachmentRole };
}
...
export function resolveAgentReferences(ctx: AgentContext, references: AgentReference[]): Record<string, unknown>[] {
  ...
  for (const reference of references) {
    ...
    if (isSelectionReference(reference)) {
      ...
      if (asset) entries.push({
        type: 'asset', id: asset.id, name: asset.name, kind: asset.kind, src: asset.src,
        durationInFrames: asset.durationInFrames, width: asset.width, height: asset.height,
        ...(reference.metadata?.attachmentRole ? { attachmentRole: reference.metadata.attachmentRole } : {}),
      });
    }
  }
  return entries;
