...
import { applyLiveKeyStatus, applyLiveModels, capabilitiesPrompt, CONFIGURED_CAPS, currentCaps, type CapabilityKey } from './capabilities';
...
assert.ok(off.includes('设置 → 本地模型 → 本地口播'), 'missing local voice points to the exact install page');
assert.ok(off.includes('do not download anything'), 'missing local voice never authorizes an Agent download');
...
// installed local voice is keyless but only lights up from an explicit, valid
// backend model/voice catalog. The prompt exposes exact IDs and still requires
// a user confirmation in auto mode.
applyLiveKeyStatus({});
applyLiveModels({});
applyLiveLocalVoiceCapabilities({
  models: [{
    ...
    license: 'Apache-2.0',
    voices: [
      { voiceId: 'zf_xiaobei', label: '小北', languageCodes: ['zh-CN'] },
      { voiceId: 'zm_yunxi', label: '云希', languageCodes: ['zh-CN', 'en-US'] },
    ],
  }],
});
const local = capabilitiesPrompt(ALL_OFF, 'auto');
assert.ok(local.includes('Voice/TTS(submit_voice'), 'installed local model enables voice without a cloud key');
assert.ok(local.includes('Local voice(provider=local)'), 'provider routing exposes local only when installed');
assert.ok(local.includes('modelId=nvidia/kokoro-82M-onnx-opt'), 'prompt shows the exact installed model ID');
assert.ok(local.includes('voiceId=zf_xiaobei') && local.includes('languageCode=zh-CN'), 'prompt shows exact voice/language choices');
assert.ok(local.includes('ask_followup_questions') && local.includes('wait for explicit confirmation'),
  'local model/voice still requires explicit confirmation in auto mode');
assert.equal(currentCaps().voice, true, 'live installed local model derives the effective voice capability');

applyLiveLocalVoiceCapabilities({ models: [{ modelId: 'bad id', status: 'installed', voices: [] }] });
const malformedLocal = capabilitiesPrompt(ALL_OFF);
assert.ok(!malformedLocal.includes('Local voice(provider=local)'), 'malformed/empty local catalog fails closed');
applyLiveLocalVoiceCapabilities(null);
