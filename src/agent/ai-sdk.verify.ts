...
assert.equal(normalizeLlmProvider('ZCode'), 'zcode');
...
assert.equal(defaultModelForProvider('zcode'), 'gemini-3.7-flash');
...
assert.equal(providerApiPath('zcode'), '/chat/completions');
...
assert.equal((await getLanguageModel('zcode', 'gemini-3.7-flash')).provider, 'zcode.chat');
...
globalThis.fetch = async (input, init) => {
  ...
  serialized.push({
    ...
    provider: headers.get('x-yolocut-provider'),
  });
...
assert.equal(
  ...
  true,
  'AI SDK must preserve rejected YoloCut results as tool-error parts',
);
