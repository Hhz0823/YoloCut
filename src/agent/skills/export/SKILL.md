---
name: export
description: Use when a YoloCut video editing or creation workflow needs export, render, download, share, final delivery, subtitle-file export, render choice, local-only asset handling, or export fallback explanation.
---

# Export

Use YoloCut's export tools for delivery. An export request should call `submit_export` (or `submit_render_job` for async), then use `track_export` for status and final delivery when the result is not returned immediately.

Default policy:

- Prefer `submit_export` when the user asks to export/share/finalize the YoloCut timeline.
...
- Do not wrap sandbox `ffmpeg` work as a YoloCut tool. Use sandbox `ffmpeg` for full video processing only when the user explicitly asks for a standalone local-file operation outside a YoloCut editing workflow. For YoloCut editing tasks, do not produce a pre-edited or flattened local render as the primary review/final deliverable; use YoloCut export.
