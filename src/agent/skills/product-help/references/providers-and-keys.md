...
| TTS / voice | `submit_voice` | Provider-specific server key(s), or an installed and user-confirmed local voice model from 设置 → 本地模型 → 本地口播 |
...
| LLM agent | chat | Configure one or more independent provider triplets: `LLM_<PROVIDER>_BASE_URL`, `LLM_<PROVIDER>_API_KEY`, and `LLM_<PROVIDER>_MODEL`. Settings exposes the complete live provider list, including direct cloud APIs, OpenAI-compatible services, Ollama/LM Studio, and the separately managed `ZCODE` local gateway. `LLM_PROVIDER` controls the initially selected chat provider. |

### ZCode Antigravity

Start ZCode Antigravity first, then open **Settings → Agent models → ZCode
Antigravity**. The automatic connection card discovers the active loopback port,
verifies ZCode's random local API key on the server, reads the advertised model
catalogue, and can select ZCode as the Agent provider in one click. The local key
is persisted in YoloCut's server-side keystore but never returned to the
browser. Manual URL/key/model entry remains available as a recovery path. The
default URL is `http://127.0.0.1:18080/v1`, but ZCode may select another port.
YoloCut never reads or stores ZCode's upstream OAuth credentials.

## Speech provider configuration

TTS provider keys:

- Local: no cloud key. The model must already be installed and exposed in the
  live local-voice catalog; the Agent never downloads it or invents a voice ID.
