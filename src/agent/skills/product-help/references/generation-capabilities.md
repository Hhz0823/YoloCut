...
Short map of generation tools → cloud or installed local providers. Use this when guiding setup or choosing a model. Exact availability is always the live **capabilities** block in the agent prompt.
...
| `inworld`, `fishaudio`, `speechify` | Account/provider voice ID plus optional model ID only; no bundled YoloCut samples |
...
| `local` | Installed on-device model/voice only; RTX 2060 minimum uses stable Kokoro; RTX 4060/5060 8–9GB recommends licensed experimental Fish S2 Q6_K; 10GB+ prefers Q8_0; model/voice/language confirmation required; result audits WebGPU/CUDA-hybrid/CPU execution |

Every provider is opt-in and requires a concrete provider-specific `voiceId`.
Only Doubao, ElevenLabs, and MiniMax have static curated choices in the voice
skill. Local choices come only from the live installed catalog; when absent,
guide `设置 → 本地模型 → 本地口播` and never auto-download. Never invent samples
for any other provider. `submit_voice` creates a media-pool asset only and does
not place it on the timeline; explicit placement uses the reviewable/undoable
edit proposal flow.
