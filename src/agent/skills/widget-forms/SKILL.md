...
`messagePrefix`), write like YoloCut is a capable video-making partner inviting
...
- Short and scannable. Use one natural sentence for `prompt` and concise
  question labels.
- Honest capability framing. YoloCut can help with many video workflows, but do
  not claim unsupported abilities or guarantee a result before inputs are known.
- The user's language and local product terms. Keep "YoloCut", "B-roll",
...
- Keep user-visible descriptions provider-neutral. Describe the voice the same
  way the native YoloCut audition UI does: gender / age range / tone / use case,
...
- Use this when the user should choose which YoloCut video workflow to start,
...
- English, Chinese, and Spanish scenario labels/descriptions/starter prompts are
  ...
  and pass those localized values in the option objects. Preserve YoloCut
...
to place directly in a project, ask them to use the YoloCut editor upload UI.
...
```json
{
  ...
  "prompt": "Choose or fill in what you have in mind so YoloCut can pick a good starting direction.",
