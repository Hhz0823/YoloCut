...
- `provider` is required. Configured choices may be `doubao`, `elevenlabs`,
  ...
  `mistral`, `cartesia`, or `local`. Cloud providers are opt-in. Local is
  available only when the capabilities prompt shows an installed local
  model/voice catalog; use only providers shown there.
...
- The curated catalog in [references/voices.md](references/voices.md) covers
  ...
  or sample catalog in YoloCut. Require a concrete voice ID from the user or
...
- Inworld, Fish Audio, and Speechify accept only `voiceId` plus optional
  ...
  these providers.
- Local accepts common `text` / `name` plus only the confirmed `modelId`,
  `voiceId`, `languageCode`, `speed`, and `devicePreference`. Never pass cloud
  controls to it. `devicePreference` is only a request; the tool result and
  asset audit report the backend-returned actual `inferenceBackend` (`webgpu`,
  `cuda-hybrid`, or `cpu`). Kokoro accepts WebGPU/CPU; experimental Fish S2
  accepts CUDA only. Never infer a fallback from the request. If CPU is
  returned, call it CPU even when WebGPU was requested.
...
- Keep the generated asset in the media pool unless the user explicitly asks to
  put it on the timeline or replace narration. For explicit placement, read the
  current timeline, then use the normal proposal/edit-session path to add the
  asset to an audio track. Do not mutate persisted project JSON, do not combine
  generation with placement as an unreviewed side effect, and do not say
  `applied` until the proposal is actually reviewed/applied. Applying the
  reviewed placement must remain one atomic undo step.
...
// Local only after the live catalog was shown and the user confirmed these
// exact installed IDs and language. Never copy these values from memory.
submit_voice({
  provider: "local",
  text: "欢迎使用本地口播。",
  modelId: confirmedInstalledModelId,
  voiceId: confirmedInstalledVoiceId,
  languageCode: confirmedLanguageCode,
  speed: 1,
...
separate providers with curated YoloCut choices, providers that require an
account-specific voice ID, and local models whose installed catalog is supplied
live in the capabilities prompt.

For `provider: "local"`, the live installed catalog is the only source of
availability. Before submitting:

1. If the catalog is empty or absent, stop and say the model is not installed.
   Guide the user to `设置 → 本地模型 → 本地口播`. Do not call a download tool,
   silently choose a cloud provider, or claim generation can proceed.
2. Show the installed model labels and exact `modelId` values, followed by the
   exact voices and supported `languageCode` values under each model. Do not
   offer models with `absent`, `downloading`, or `error` status.
3. Load `widget-forms` and use `ask_followup_questions` to get an explicit
   model/voice/language confirmation. Use only a `previewUrl` included in that
   same live catalog; never derive a sample path.
4. Wait for the response, then copy the confirmed IDs exactly into
   `submit_voice`. A broad request such as "use a local female voice" is not a
   confirmation and is not a `voiceId`.

The stable first local model semantics are NVIDIA Kokoro 82M ONNX
(`nvidia/kokoro-82M-onnx-opt`, Apache-2.0), with Windows/Turing/RTX 2060 as the
documented hardware baseline. This is reference information, not permission to
assume installation: offer it only when the exact model appears as `installed`
in the live catalog.

The product hardware recommendation is: RTX 2060/6 GB = stable Kokoro minimum;
RTX 4060 or RTX 5060 with 8–9 GB = experimental Fish Audio S2 Pro through
`s2.cpp` + GGUF `Q6_K`; RTX 40/50-series GPUs with at least 10 GB = `Q8_0`.
Fish S2 is usable only when the live catalog reports both installed weights and
an available pinned runtime. Its Fish Audio Research License permits research
and non-commercial use; commercial use needs a separate written license. Never
accept that license, download 4–6 GB weights, or substitute Fish S2 on the
user's behalf. The cloud `provider: "fishaudio"` is a separate provider path.
...
For a provider without a curated YoloCut catalog, ask for a free-text,
...
| --- | --- | --- |
| `provider` | Cloud providers above, or `local` | Required; configured/installed choices only |
| `text` | Text to synthesize | Required |
| `voiceId` | Concrete provider-specific voice ID | Required except MiniMax timbre mix |
| `modelId` | Provider model override or exact installed local model ID | Required for local |
| `speed` | Speech speed | ElevenLabs, MiniMax, OpenAI, Cartesia, local Kokoro; Fish S2 currently fixed at 1.0 |
| `languageCode` | Language hint/code or confirmed local catalog language | Required for local |
| `devicePreference` | `auto`, `webgpu`, `cuda`, or `cpu` | Local only; Kokoro uses WebGPU/CPU, Fish S2 uses CUDA; result reports actual backend |
...
confirmed by the user and have no bundled YoloCut samples. Local model and
voice IDs come only from the live installed catalog in capabilities; the
reference list below does not prove that a local model is installed.
...
- For local narration, show the live installed model/voice/language options and
  require explicit confirmation even in auto mode. Never silently download or
  substitute a model.
...
8. Never call `provider: "local"` without a non-empty installed catalog and an
   explicit user confirmation of exact `modelId`, `voiceId`, and
   `languageCode`.
9. Never report `devicePreference` as the executed backend. Report only the
   returned `inferenceBackend` (`webgpu`, `cuda-hybrid`, or `cpu`). Do not
   invent a fallback reason; a CPU result stays labeled CPU.
10. Never place a generated voice asset on the timeline unless the user asks;
    placement/replacement must go through a reviewable, atomic, undoable edit
    proposal.
