...
YoloCut preset/sample catalog. For those providers, require a concrete
...
Local availability is dynamic and is **not** determined by this file. The live
capabilities prompt must contain an installed model entry and its exact voices.
Show that list to the user and wait for explicit confirmation of `modelId`,
`voiceId`, and `languageCode` before calling `submit_voice`. If the list is
empty, guide the user to `设置 → 本地模型 → 本地口播`; never download a model on
the user's behalf.

The stable first-model reference is:

- Model: `nvidia/kokoro-82M-onnx-opt`
- Revision: `2c9213187a1925bd87478540b6c8cda1a49a8d52`
- Runtime: NVIDIA Kokoro 82M ONNX
- License: Apache-2.0
- Documented baseline: native Windows, NVIDIA Turing, including RTX 2060

The current Windows Node runtime contract is intentionally `webgpu | cpu`.
With `onnxruntime-node@1.27.0` and the optimized NVIDIA Kokoro model, the
validated ORT profile placed 2,186 nodes on WebGPU. Both tested DirectML model
variants failed, and CUDA is unavailable in this Windows Node path, so neither
name is a valid request preference or result backend. This baseline does not
replace per-run audit: always report the exact backend returned for that job.

Its Chinese voice IDs include `zf_xiaobei`, `zf_xiaoni`, `zf_xiaoxiao`,
`zf_xiaoyi`, `zm_yunjian`, `zm_yunxi`, `zm_yunxia`, and `zm_yunyang`. These are
reference identifiers, not an installed catalog. Do not offer any of them
unless the live model response lists that exact voice under an `installed`
model. Use only the `languageCodes`, labels, and optional `previewUrl` supplied
by that same live response; do not infer a language code from a voice prefix or
derive a sample URL.

Experimental higher-tier local references may also appear in the live catalog:

- RTX 4060 / RTX 5060 with 8–9 GB: Fish Audio S2 Pro, `s2.cpp` + GGUF `Q6_K`
- RTX 40/50 series with at least 10 GB: Fish Audio S2 Pro, `s2.cpp` + GGUF `Q8_0`

These are not part of the stable AGPL model baseline. Both weights and the
community alpha runtime use the Fish Audio Research License: research and
non-commercial use is permitted, while commercial use requires separate
written permission. An installed GGUF is still unavailable when the pinned
external runtime integrity gate fails. The first experimental catalog exposes
only `random-zh`, whose timbre may change between calls; do not describe it as
a persistent cloned voice. Do not enable cloning or save a voice profile
without a separate voice-consent workflow.

Local generation accepts only the confirmed IDs/language plus `speed` and
`devicePreference`. The successful tool result and media asset audit record the
backend-returned actual `inferenceBackend` (`webgpu`, `cuda-hybrid`, or `cpu`)
and model. Never
turn a requested WebGPU preference into an execution or fallback claim; a CPU
result must be shown as CPU, and a fallback reason is recorded only when the
backend explicitly supplies one.
