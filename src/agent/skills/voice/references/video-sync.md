...
9. Place each voiceover segment only after the actual-duration fit check passes
   and only when the user explicitly requested timeline placement/replacement.
   Use the normal reviewable proposal/edit-session path so all selected audio
   placements/mix changes apply as one atomic history action and one undo
   restores the pre-placement timeline. Place it at the matching visual start
   time from the current sync map. If original audio, music, or sound effects
   exist, decide whether to mute, duck, replace, or mix them based on user
   intent. Generation itself remains a media-pool action and must not silently
   place a clip.
