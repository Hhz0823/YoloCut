...
Grounded in YoloCut’s voice tool + server adapter (`server/plugins/voice.ts` → MiniMax `t2a_v2`). Preset table: [voices.md](voices.md) § MiniMax.
...
| Streaming | `stream`; `excludeAggregatedAudio`; `forceCbr` only with streamed MP3. YoloCut still persists one local asset |
