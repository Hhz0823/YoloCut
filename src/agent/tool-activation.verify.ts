...
const portableActivation = prepareMessagesForProvider([{
  ...
  content: [{
    ...
    providerOptions: {
      ...
      yolocut: { activatedTools: ['web_crawl'] },
    },
  }],
}], 'anthropic', 'openai');
assert.deepEqual(portableActivation, [{
  ...
  content: [{
    type: 'text',
    text: 'ToolSearch checkpoint.',
    providerOptions: { yolocut: { activatedTools: ['web_crawl'] } },
  }],
}], 'provider switches retain only portable YoloCut activation metadata');
const migratedPortableActivation = prepareMessagesForProvider([{
  role: 'assistant',
  content: [{
    type: 'text',
    text: 'Historical ToolSearch checkpoint.',
    providerOptions: {
      [LEGACY_PERSISTENCE_IDS.agentProviderOptionsKey]: { activatedTools: ['web_crawl'] },
    },
  }],
}], 'anthropic', 'openai');
const migratedPortableContent = migratedPortableActivation[0]?.content;
assert.ok(Array.isArray(migratedPortableContent));
assert.deepEqual(
  (migratedPortableContent[0] as { providerOptions?: unknown })?.providerOptions,
  { yolocut: { activatedTools: ['web_crawl'] } },
  'historical activation metadata normalizes to YoloCut',
);
assert.deepEqual(
...
const injected = new ToolActivation(catalog, [
  { role: 'assistant', content: '[YoloCut activated tools: web_crawl]' },
  { role: 'user', content: '继续' },
]);
