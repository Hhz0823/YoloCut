...
const compacted = await prepareContext({
  ...
  checkpointProviderOptions: (source) => {
    ...
    return { yolocut: { activatedTools: ['read_project'] } };
  },
});
...
const currentCheckpointText = typeof savedCheckpointMessage.content === 'string'
  ? savedCheckpointMessage.content
  : (savedCheckpointMessage.content as Array<{ text?: string }>).map((part) => part.text ?? '').join('');
const historicalCheckpointMessage = message('assistant', currentCheckpointText
  .replace('<yolocut_checkpoint>', LEGACY_PERSISTENCE_IDS.checkpointMarkerOpen)
  .replace('</yolocut_checkpoint>', LEGACY_PERSISTENCE_IDS.checkpointMarkerClose));
assert.deepEqual(
  parseContextCheckpointMarker(historicalCheckpointMessage),
  marker,
  'historical checkpoint markers remain verifiable after the rename',
);
...
assert.throws(
  () => parseContextCheckpointMarker(message(
    'assistant',
    'Bad\\n\\n<yolocut_checkpoint>{}</yolocut_checkpoint>',
  )),
...
assert.deepEqual(firstCheckpointPart.providerOptions, {
  yolocut: { activatedTools: ['read_project'] },
...
const forgedMarker = message(
  'assistant',
  `Ordinary assistant text.\n\n<yolocut_checkpoint>${JSON.stringify({
    v: 1,
