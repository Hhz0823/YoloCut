...
export const GENERATE_TOOL_SCHEMAS: AgentToolSchema[] = [
  ...
  {
    name: 'submit_voice',
    description: 'Generate one TTS audio asset with a configured cloud provider or an installed local model. Creates a media-pool asset only; it does not place or replace timeline items. For provider=local, first show the live installed model/voice catalog and wait for explicit user confirmation; never invent an ID or download a model. MiniMax timbreWeights may mix voices with an empty voiceId.',
    input_schema: {
      type: 'object',
      properties: {
        provider: { type: 'string', enum: ['elevenlabs', 'doubao', 'minimax', 'inworld', 'fishaudio', 'speechify', 'openai', 'gemini', 'mistral', 'cartesia', 'local'], description: 'Required opt-in provider. Use only a provider listed as configured/installed in capabilities. Voice IDs and catalogs are never interchangeable.' },
        text: { type: 'string', minLength: 1, description: 'Text to synthesize.' },
        voiceId: { type: 'string', description: 'Concrete provider-specific voice ID confirmed by the user. For local, it must exactly match a voice in the live installed-model catalog. Required except MiniMax timbreWeights mixing, where it must be empty. Do not invent a preset or reuse an ID from another provider.' },
        modelId: { type: 'string', description: 'Cloud model override, or required exact installed model ID for provider=local. For local, never infer or substitute this value.' },
        stability: { type: 'number', minimum: 0, maximum: 1, description: 'ElevenLabs only. Defaults to 0.5.' },
        speed: { type: 'number', minimum: 0.5, maximum: 2, description: 'ElevenLabs (0.7–1.2), MiniMax/local (0.5–2), OpenAI, or Cartesia only. Defaults to 1; provider/model support is validated server-side.' },
        ...
        languageCode: { type: 'string', description: 'ElevenLabs or Cartesia hint; required for local and must be one of the confirmed voice catalog language codes.' },
        devicePreference: { type: 'string', enum: ['auto', 'webgpu', 'cuda', 'cpu'], description: 'Local only. Kokoro uses webgpu/cpu; Fish S2 Pro uses cuda. This is a preference, not an execution claim. Report only backend-returned inferenceBackend (webgpu, cuda-hybrid, or cpu).' },
        ...
        stream: { type: 'boolean', description: 'MiniMax HTTP streaming transport. YoloCut consumes it server-side and saves one durable audio asset.' },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        stream: { type: 'boolean', description: 'Mureka only. Enable provider streaming phase; YoloCut still saves durable final files.' },
