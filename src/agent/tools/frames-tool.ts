...
/** Raw frame evidence for a second local vision pass. This deliberately skips
 * maybeDescribeFramesResult so callers can route the JPEG bytes to the pinned
 * on-device model instead of whichever main Agent provider is selected. */
export async function renderAssetFrameEvidence(args: Args, ctx: AgentContext): Promise<unknown> {
  return viewAssetFrames(args, ctx);
}
