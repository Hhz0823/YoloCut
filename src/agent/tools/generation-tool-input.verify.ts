...
const localVoice = buildSubmitVoiceArgs({
  provider: 'local',
  text: '本地口播',
  name: '离线旁白',
  ...
  stability: 0.5,
  outputFormat: 'mp3',
  instructions: 'invented cloud control',
  emotion: 'happy',
  pitch: 2,
  volume: 2,
  sampleRate: 44_100,
...
assert.equal(localVoice.provider, 'local');
assert.equal(localVoice.modelId, 'nvidia/kokoro-82M-onnx-opt');
assert.equal(localVoice.voiceId, 'zf_xiaobei');
assert.equal(localVoice.languageCode, 'zh-CN');
assert.equal(localVoice.speed, 1.05);
assert.equal(localVoice.devicePreference, 'webgpu');
assert.deepEqual(
  Object.keys(localVoice).filter((key) => localVoice[key as keyof typeof localVoice] !== undefined).sort(),
  ['devicePreference', 'languageCode', 'modelId', 'name', 'provider', 'speed', 'text', 'voiceId'].sort(),
  'local input parser must whitelist only common fields plus local model/voice/language/speed/device preference',
);
assert.equal(
  buildSubmitVoiceArgs({ ...localVoice, devicePreference: 'cuda' }).devicePreference,
  'cuda',
  'Fish S2 Pro may request the explicit CUDA runtime preference',
);
for (const removedPreference of ['directml', 'cuda-hybrid', 'gpu']) {
  assert.equal(
    buildSubmitVoiceArgs({ ...localVoice, devicePreference: removedPreference }).devicePreference,
    undefined,
    `${removedPreference} is not a valid local voice preference and must not reach the backend`,
  );
}
