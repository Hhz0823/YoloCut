...
const submitVoiceHandler: Handler = async (args, ctx) => {
  ...
  addAsset(ctx, asset);
  const localAudit = localVoiceAuditFromAsset(asset);
  return {
    ...
    ...(localAudit ? {
      modelId: localAudit.modelId,
      modelRevision: localAudit.modelRevision,
      languageCode: localAudit.languageCode,
      speed: localAudit.speed,
      requestedDevicePreference: localAudit.requestedDevicePreference,
      inferenceBackend: localAudit.inferenceBackend,
      fallbackReason: localAudit.fallbackReason,
    } : {}),
  };
};
