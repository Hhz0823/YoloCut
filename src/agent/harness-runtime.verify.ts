...
import { executeYoloCutTool, type CodexToolExecution } from './codex/runtime';
...
async function executeWithRecorder(
  ...
  return executeYoloCutTool(aspectSchema, { ratio: '9:16' }, {
...
async function executeInstallSkill(
  ...
  return executeYoloCutTool(installSkillSchema, { repo: 'owner/skill' }, {
...
async function verifyAbortFence(): Promise<void> {
  ...
  const pending = executeYoloCutTool(aspectSchema, { ratio: '9:16' }, {
...
async function verifyYoloSkipsAllGuards(): Promise<void> {
  ...
  const execution = await executeYoloCutTool(aspectSchema, { ratio: '9:16' }, {
