...
import { executeYoloCutTool } from './codex/runtime';
import {
...
async function executeDesign(
  ...
  return executeYoloCutTool(designSchema, args, {
...
async function verifyExplicitTranscriptionProviders(): Promise<void> {
  ...
  await executeYoloCutTool(transcribeSchema, { track: 'A2', provider: 'local' }, {
    ctx,
    settings,
...
async function verifySettingBackedTranscriptionProvider(saved: Record<string, string>): Promise<void> {
  ...
  await executeYoloCutTool(transcribeSchema, { track: 'A2' }, {
    ctx,
    settings,
