...
function isActivationCheckpoint(message: ModelMessage): boolean {
  ...
  return message.content.some((part) => {
    ...
    const activation = Reflect.get(options, 'yolocut')
      ?? Reflect.get(options, LEGACY_PERSISTENCE_IDS.agentProviderOptionsKey);
    const names: string[] = [];
    collectActivatedNames(activation, names);
...
export function activationProviderOptions(names: readonly string[]): ProviderOptions | undefined {
  ...
  return activatedTools.length ? { yolocut: { activatedTools } } : undefined;
}
