...
import { LOCAL_VOICE_SETTINGS_PATH } from '../../shared/local-voice';
import { availableLocalVoiceModels } from '../generate/local-voice-status';
...
export function currentCaps(): Record<CapabilityKey, boolean> {
  const base = liveCaps ?? CONFIGURED_CAPS;
  return availableLocalVoiceModels().length > 0 && !base.voice ? { ...base, voice: true } : base;
}
...
const CAP_PROVIDERS: Partial<Record<CapabilityKey, ProviderRow[]>> = {
  ...
  voice: [
    ...
    { label: 'Local voice', arg: 'local', argKey: 'provider', need: [], localVoice: true },
  ],
  video: [
...
function providerSuffix(cap: CapabilityKey, mode: ApprovalMode): string {
  ...
  const on = rows.filter((r) => r.localVoice
    ? availableLocalVoiceModels().length > 0
    : Boolean(liveKeys) && r.need.some((group) => group.every(has)));
...
function localVoiceStatusPrompt(): string {
  const models = availableLocalVoiceModels();
  if (models.length === 0) {
    return `\nLocal voice (provider=local): unavailable. Do not call it and do not download anything. If requested, guide the user to ${LOCAL_VOICE_SETTINGS_PATH}.`;
  }
  const catalog = models.map((model) => {
    const voices = model.voices.map((voice) => (
      `${voice.label}(voiceId=${voice.voiceId}; languageCode=${voice.languageCodes.join('|')}`
      + `${voice.previewUrl ? `; previewUrl=${voice.previewUrl}` : ''})`
    )).join(', ');
    const revision = model.revision ? `; revision=${model.revision}` : '';
    const license = model.license ? `; license=${model.license}` : '';
    return `${model.label}(modelId=${model.modelId}${revision}${license}; voices: ${voices})`;
  }).join(' | ');
  return `\nLocal voice installed catalog (exact IDs only): ${catalog}.`
    + '\nBefore every first provider=local generation selection, show the applicable installed model and voice options to the user with ask_followup_questions, include their supported languageCode values, and wait for explicit confirmation. A broad request such as "use a local voice" is not confirmation. Never invent a model/voice/sample or download a model.';
}
...
export function capabilitiesPrompt(
  ...
  for (const r of CAP_ROWS) {
    const available = caps[r.key] || (r.key === 'voice' && availableLocalVoiceModels().length > 0);
    if (available) on.push(`${r.label}(${r.tool}${providerSuffix(r.key, mode)})`);
