...
async function main(): Promise<void> {
  ...
  const previousEnv = process.env.YOLOCUT_SQLITE_STORE;
  process.env.HOME = root;
  process.env.USERPROFILE = root;
  process.env.YOLOCUT_SQLITE_STORE = '1';
  let closeStores = (): void => undefined;

  try {
    const { initializeSqliteProjectStore, resetSqliteStoreForTests, SQLITE_STORE_ENV } = await import('./sqlite-store.ts');
    ...
    closeStores();
    if (previousEnv === undefined) delete process.env.YOLOCUT_SQLITE_STORE;
    else process.env.YOLOCUT_SQLITE_STORE = previousEnv;
