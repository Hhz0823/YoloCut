...
function llmProbe(provider: LlmProvider): ProbeDef {
  ...
  return {
    ...
    run: (get) => {
      ...
      const request = isLoopbackLlmProvider(provider) ? fetch : fetchWithProxy;
      return request(`${root}/models`, { signal: t(), headers });
    },
...
const atlasMusicProbe: ProbeDef = {
  ...
  run: async (get) => {
    ...
    const response = await fetchWithProxy(`${root}/model/prediction/yolocut-credential-probe`, {
