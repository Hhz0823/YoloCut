...
const fixture = await mkdtemp(join(tmpdir(), 'yolocut-keystore-profile-'));
const checkout = join(fixture, 'checkout');
const home = join(fixture, 'home');
const profileRoot = join(home, '.yolocut', 'dev-profiles', profileId);
const checkoutEnv = join(checkout, '.env.local');
const profileEnv = join(profileRoot, 'settings.env');
const previousCwd = process.cwd();
const previousHome = process.env.HOME;
const previousUserProfile = process.env.USERPROFILE;
const previousProfile = process.env.YOLOCUT_DEV_PROFILE_ID;
...
try {
  ...
  process.env.YOLOCUT_DEV_PROFILE_ID = profileId;
  // Intentional module-boundary test: cwd, cross-platform home variables, and
  // profile ID must be set before initialization.
  ...
  if (previousProfile === undefined) delete process.env.YOLOCUT_DEV_PROFILE_ID;
  else process.env.YOLOCUT_DEV_PROFILE_ID = previousProfile;
