...
const NEVER_RETURN = new Set(['set-cookie']);

function transportCode(error: Error): string {
  ...
  return (req, res) => {
    ...
    const upstream = doRequest({
      ...
      if (status >= 400 && route.errorMessage) {
        // Never record provider bodies: they may contain request excerpts,
        // account details, OAuth diagnostics, or echoed credentials.
        upRes.resume();
        console.warn(`[proxy] upstream returned HTTP ${status}`);
      ...
      for (const [k, v] of Object.entries(upRes.headers)) {
        if (!HOP_BY_HOP.has(k.toLowerCase()) && !NEVER_RETURN.has(k.toLowerCase()) && v !== undefined) outHeaders[k] = v;
      }
    ...
    upstream.on('error', (err) => {
      if (!res.headersSent) {
        console.warn(`[proxy] upstream transport failure (${transportCode(err)})`);
        res.writeHead(502, { 'Content-Type': 'application/json' });
        const message = route.errorMessage?.(502, req) ?? 'upstream service unavailable';
        res.end(JSON.stringify(route.errorMessage ? { error: { message } } : { error: message }));
