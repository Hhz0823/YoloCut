...
const stageIndex = pluginNames.indexOf('yolocut-export-stage');
const exportIndex = pluginNames.indexOf('yolocut-export');
assert.ok(stageIndex >= 0 && exportIndex >= 0 && stageIndex < exportIndex, 'stage route must precede /export catch-all');

const directory = await mkdtemp(join(tmpdir(), 'yolocut-export-stage-cleanup-'));
try {
  const now = Date.now();
  const staleStage = 'yolocut-export-stage-00000000-0000-4000-8000-000000000001.mp4';
  const stalePartial = 'yolocut-export-stage-00000000-0000-4000-8000-000000000002.webm.part';
  const freshStage = 'yolocut-export-stage-00000000-0000-4000-8000-000000000003.webm';
  const prefixedUserFile = 'yolocut-export-stage-project.mp4';
  await Promise.all([
