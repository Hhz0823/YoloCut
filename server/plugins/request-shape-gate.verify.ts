...
const zcodeConnect = req({ url: '/api/zcode/connect' });
assert.equal(requestShapeAllowed(zcodeConnect), true, 'same-origin ZCode connect write allowed');
const crossSiteZcode = req({
  url: '/api/zcode/connect',
  headers: { ...ok.headers, 'sec-fetch-site': 'cross-site' },
});
assert.equal(requestShapeAllowed(crossSiteZcode), false, 'cross-site ZCode connect write blocked');
