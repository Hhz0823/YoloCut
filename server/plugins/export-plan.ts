...
export interface ExportPlan {
  ...
  /** Requested final output scale (metadata + exact FFmpeg conform target). */
  scale: number;
  /** Uniform scale accepted by Remotion; may differ from scale. */
  renderScale: number;
...
export function planExport(body: ExportRequest | null): ExportPlan {
  ...
  return {
    ...
    renderScale: remotionExportScale(state, body?.resolution),
    retimeFps,
