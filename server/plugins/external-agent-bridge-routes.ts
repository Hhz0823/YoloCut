...
import type { mcpConnectionManifest, mcpTools } from '../external-agent/mcp.ts';
import {
  abortUploadReceipt,
  claimUploadReceipt,
  commitUploadReceipt,
...
const REGISTRATION_CAPABILITY_HEADER = 'x-yolocut-editor-registration';
...
export interface BridgeOperations {
  ...
  mcpConnectionManifest: typeof mcpConnectionManifest;
}
...
export async function routeExternalAgentBridge(
  ...
  } else if (req.method === 'GET' && url.pathname === '/diagnostics') {
    sendBridgeJson(res, 200, operations.mcpConnectionManifest());
  } else {
