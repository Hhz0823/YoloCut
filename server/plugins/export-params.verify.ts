...
import { remotionExportScale } from '../../src/export/mediaSettings.ts';
...
assert.equal(remotionExportScale({ width: 1920, height: 1080 }, '480p'), 1,
  'fractional 480p output renders at a legal scale before FFmpeg conform');
assert.equal(remotionExportScale({ width: 1920, height: 1080 }, '720p'), 2 / 3,
  'an exact integer Remotion scale needs no intermediate render size');
