...
const rootDir = await mkdtemp(join(tmpdir(), 'yolocut-extensions-'));
const harness = await createHarness(rootDir);
const pack = {
  format: LEGACY_PORTABLE_FORMATS.plugin,
  id: 'demo-pack',
  name: 'Demo Pack',
  version: '..',
...
try {
  ...
  let body = await response.json() as { packs: Array<{ id: string; enabled: boolean; format: string }> };
  assert.equal(body.packs[0]?.id, pack.id);
  assert.equal(body.packs[0]?.enabled, true);
  assert.equal(body.packs[0]?.format, 'yolocut-plugin@1', 'stored manifests normalize to YoloCut');
