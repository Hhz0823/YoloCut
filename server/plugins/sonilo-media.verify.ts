...
const original = {
  dataDir: process.env.YOLOCUT_DATA_DIR,
  profileId: process.env.YOLOCUT_DEV_PROFILE_ID,
  httpProxy: process.env.HTTP_PROXY,
  httpsProxy: process.env.HTTPS_PROXY,
  lowerHttpProxy: process.env.http_proxy,
  lowerHttpsProxy: process.env.https_proxy,
};
process.env.YOLOCUT_DATA_DIR = root;
...
const provider = createServer(async (req, res) => {
  ...
  assert.equal(req.headers['user-agent'], 'YoloCut');
  const chunks: Buffer[] = [];
...
try {
  ...
  if (original.dataDir === undefined) delete process.env.YOLOCUT_DATA_DIR;
  else process.env.YOLOCUT_DATA_DIR = original.dataDir;
  if (original.profileId === undefined) delete process.env.YOLOCUT_DEV_PROFILE_ID;
  else process.env.YOLOCUT_DEV_PROFILE_ID = original.profileId;
