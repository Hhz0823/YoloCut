// install_skill: download a GitHub skill repo into ~/.yolocut/skills/<slug>/
...
function apiHeaders(): Record<string, string> {
  const headers: Record<string, string> = { Accept: 'application/vnd.github+json', 'User-Agent': 'yolocut-skill-install' };
...
export async function installGitHubSkill(
  ...
  return { slug, installedAt: join('~', '.yolocut', 'skills', slug), files: result.files, source: result.source };
}

export function skillInstallPlugin(): Plugin {
  return {
    name: 'yolocut-skill-install',
