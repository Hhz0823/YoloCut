...
const KOKORO_REVISION = '2c9213187a1925bd87478540b6c8cda1a49a8d52';
const TOKENIZER_REVISION = '6cc0f0d2ebe369a68b0df87c2b65c1af8c0ac3e3';
...
assert.deepEqual(
  parseTarget(`/nvidia/kokoro-82M-onnx-opt/resolve/${KOKORO_REVISION}/kokoro-82m-v1.0.onnx`),
  { modelId: 'nvidia/kokoro-82M-onnx-opt', revision: KOKORO_REVISION, filePath: 'kokoro-82m-v1.0.onnx' },
);
assert.deepEqual(
  parseTarget(`/onnx-community/Kokoro-82M-v1.1-zh-ONNX/resolve/${TOKENIZER_REVISION}/tokenizer.json`),
  { modelId: 'onnx-community/Kokoro-82M-v1.1-zh-ONNX', revision: TOKENIZER_REVISION, filePath: 'tokenizer.json' },
  'secondary tokenizer source is whitelisted but installs under the primary pack root',
);
const sourceUrls = downloadSourceUrlsForVerify({
  modelId: 'onnx-community/Kokoro-82M-v1.1-zh-ONNX',
  revision: TOKENIZER_REVISION,
  filePath: 'tokenizer.json',
});
assert.ok(sourceUrls.every((source) => !/[?&]Revision=(?:main|master)(?:&|$)/i.test(source.url)));
assert.match(sourceUrls.find((source) => source.name === 'modelscope')?.url ?? '', new RegExp(TOKENIZER_REVISION));
...
const rejectedPaths = [
  ...
  `/onnx-community/Kokoro-82M-v1.1-zh-ONNX/resolve/${TOKENIZER_REVISION}/..%2Ftokenizer.json`,
  `/onnx-community/Kokoro-82M-v1.1-zh-ONNX/resolve/main/tokenizer.json`,
...
const directory = await mkdtemp(join(tmpdir(), 'yolocut-hf-proxy-'));
...
if (!process.env.HF_PROXY_SKIP_INTEGRATION) {
  const integrationDir = await mkdtemp(join(tmpdir(), 'yolocut-hf-proxy-integration-'));
  try {
    __resetModelMissingState();
...
assert.equal(modelCacheDir(), join(directory, '.yolocut', 'asr-models'));
