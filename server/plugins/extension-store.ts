...
const DEFAULT_ROOT_DIR = join(runtimeProfile().rootDir, 'plugins');
const FORMAT = 'yolocut-plugin@1';
...
function validPack(value: unknown): StoredPack | null {
  if (!isRecord(value)
    || (value.format !== FORMAT && value.format !== LEGACY_PORTABLE_FORMATS.plugin)
    || !ID_RE.test(String(value.id ?? ''))) return null;
