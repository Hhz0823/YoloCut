...
export async function setupGenerationJobsFixture(): Promise<GenerationJobsFixture> {
  const storeRoot = await mkdtemp(join(tmpdir(), 'yolocut-generation-jobs-'));
  const storePath = join(storeRoot, 'operations.json');
  const previousStorePath = process.env.YOLOCUT_GENERATION_JOB_STORE;
  process.env.YOLOCUT_GENERATION_JOB_STORE = storePath;
  ...
  const deleteExportName = `yolocut-export-job-${deleteAssetId}.mp4`;
  const expiryExportName = `yolocut-export-job-${expiryAssetId}.mp4`;
...
export async function cleanupGenerationJobsFixture(fixture: GenerationJobsFixture): Promise<void> {
  if (fixture.previousStorePath === undefined) delete process.env.YOLOCUT_GENERATION_JOB_STORE;
  else process.env.YOLOCUT_GENERATION_JOB_STORE = fixture.previousStorePath;
