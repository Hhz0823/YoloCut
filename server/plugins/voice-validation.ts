...
import { isLocalVoiceDevicePreference } from '../../shared/local-voice.ts';
...
const LOCAL_ALLOWED_FIELDS = new Set([
  'provider', 'text', 'voiceId', 'modelId', 'languageCode', 'speed', 'devicePreference', 'name',
]);

function validateLocal(input: VoiceRequest): void {
  const unknown = Object.keys(input).filter((key) => (
    !LOCAL_ALLOWED_FIELDS.has(key) && (input as Record<string, unknown>)[key] !== undefined
  ));
  if (unknown.length) throw new Error(`Local voice does not accept parameter(s): ${unknown.join(', ')}`);
  const modelId = String(input.modelId ?? '').trim();
  const languageCode = String(input.languageCode ?? '').trim();
  if (!modelId) throw new Error('local modelId is required and must come from the installed-model catalog');
  if (!/^[A-Za-z0-9][A-Za-z0-9._/@:+-]{0,199}$/.test(modelId)) throw new Error('invalid local modelId');
  if (!languageCode) throw new Error('local languageCode is required and must match the confirmed voice catalog');
  if (!/^[A-Za-z]{2,3}(?:-[A-Za-z0-9]{2,8})*$/.test(languageCode)) throw new Error('invalid local languageCode');
  range(input.speed, 0.5, 2, 'speed');
  if (input.devicePreference !== undefined && !isLocalVoiceDevicePreference(input.devicePreference)) {
    throw new Error('local devicePreference must be auto, webgpu, cuda, or cpu');
  }
}
...
export function validateVoiceRequest(input: VoiceRequest): ValidVoiceRequest {
  ...
  if (provider !== 'elevenlabs' && provider !== 'doubao' && provider !== 'minimax'
    ...
    && provider !== 'local' && !isAiProvider(provider)) {
  ...
  if (provider !== 'local' && input.devicePreference !== undefined) {
    throw new Error(`${provider} does not accept local-only devicePreference`);
  }
  if (provider === 'local') validateLocal(input);
  else if (provider === 'elevenlabs') validateEleven(input);
  ...
  if (provider === 'local') {
    return {
      ...
      speed: input.speed ?? 1,
      devicePreference: input.devicePreference ?? 'auto',
      outputFormat: 'wav',
      sampleRate: 24_000,
      audioFormat: 'wav',
      channel: 1,
