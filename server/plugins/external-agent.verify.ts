...
const operations = {
  ...
  mcpConnectionManifest: () => {
    calls.mcpConnectionManifest += 1;
    return { version: 1, server: 'yolocut' } as ReturnType<BridgeOperations['mcpConnectionManifest']>;
  },
...
const bridgeRequests: Array<[keyof BridgeOperations, string, RequestInit | undefined]> = [
  ...
  ['mcpConnectionManifest', '/diagnostics', undefined],
];
...
const originalToken = process.env.YOLOCUT_MCP_TOKEN;
const originalEditorUrl = process.env.YOLOCUT_EDITOR_URL;
try {
  process.env.YOLOCUT_MCP_TOKEN = 'mcp-secret';
  delete process.env.YOLOCUT_EDITOR_URL;
  ...
  process.env.YOLOCUT_EDITOR_URL = 'https://editor.example';
  ...
  const wrongCapabilityHeaders = {
    'X-YoloCut-Editor-Registration': 'w'.repeat(43),
  };
  ...
  for (const [operation, path, init] of bridgeRequests.slice(1)) {
    const response = await requestBridge(path, {
      ...
      headers: {
        ...
        'X-YoloCut-Editor-Registration': registrationCapability,
      },
    });
    assert(
  ...
  process.env.YOLOCUT_EDITOR_URL = origin;
  ...
  delete process.env.YOLOCUT_MCP_TOKEN;
  delete process.env.YOLOCUT_EDITOR_URL;
  ...
  if (originalToken === undefined) delete process.env.YOLOCUT_MCP_TOKEN;
  else process.env.YOLOCUT_MCP_TOKEN = originalToken;
  if (originalEditorUrl === undefined) delete process.env.YOLOCUT_EDITOR_URL;
  else process.env.YOLOCUT_EDITOR_URL = originalEditorUrl;
  server.close();
