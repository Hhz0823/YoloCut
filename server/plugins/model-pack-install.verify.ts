...
import {
  handleModelPackRequest,
  modelPackLicenseAcceptanceIssue,
  modelPackMutationRequestError,
} from './model-packs';
...
const fishPack = modelPackDefinition('fish-s2-pro-q8-local');
assert(fishPack);
assert.match(modelPackLicenseAcceptanceIssue(fishPack, undefined) ?? '', /商业使用.*书面许可/);
assert.equal(
  modelPackLicenseAcceptanceIssue(fishPack, fishPack.licensePolicy?.acceptanceId),
  undefined,
  'the exact pinned agreement id unlocks only the explicit install request',
);
const openPack = modelPackDefinition('kokoro-tts-local');
assert(openPack);
assert.equal(modelPackLicenseAcceptanceIssue(openPack, undefined), undefined);
...
const root = await mkdtemp(join(tmpdir(), 'yolocut-model-pack-'));
try {
