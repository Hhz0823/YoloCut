...
export function registerExportRoute(server: ViteDevServer): void {
  server.middlewares.use('/export', async (req, res) => {
    ...
    let resizedOutput: string | null = null;
    ...
    try {
      ...
      const { state, format, media, frameRange, filename, scale, renderScale } = plan;
      const finalOutput = join(tmpdir(), `yolocut-export-${randomUUID()}.${media.ext}`);
      ...
      await withExportPermit(async () => {
        ...
        const renderSize = exportOutputSize(state, renderScale);
        if (format === 'video'
          && (outputSize.width !== renderSize.width || outputSize.height !== renderSize.height)) {
          resizedOutput = `${finalOutput}.resized.${media.ext}`;
          await resizeVideo(
            finalOutput,
            resizedOutput,
            outputSize.width,
            outputSize.height,
            media.codec as 'h264' | 'vp8' | 'prores',
            plan.videoBitrate ?? resolveH264TargetBitrate({ ...outputSize, fps: state.fps }),
            requestAbort.controller.signal,
          );
          requestAbort.controller.signal.throwIfAborted();
          await unlink(finalOutput).catch(() => {});
          await rename(resizedOutput, finalOutput);
          resizedOutput = null;
        }
      ...
      const cleanupStatus = await cleanupExportOutputs([outputLocation, resizedOutput, retimedOutput]);
      outputLocation = null;
      resizedOutput = null;
      retimedOutput = null;
      ...
      if (resizedOutput) await unlink(resizedOutput).catch(() => {});
