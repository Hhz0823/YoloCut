...
const directory = await mkdtemp(join(tmpdir(), 'yolocut-extract-audio-'));
const previousProbe = process.env.YOLOCUT_FFPROBE;
...
try {
  ...
  server = await createServer({
    ...
    // Focused HTTP route test: file watching is unnecessary and Windows/libuv
    // can abort on temp-directory long-path versus 8.3 alias events.
  ...
  process.env.YOLOCUT_FFPROBE = join(directory, 'missing-ffprobe');
  ...
  if (previousProbe === undefined) delete process.env.YOLOCUT_FFPROBE;
  else process.env.YOLOCUT_FFPROBE = previousProbe;
