...
import { mkdir, mkdtemp, readFile, readdir, rename, rm, stat, statfs, unlink, utimes, writeFile } from 'node:fs/promises';
...
async function previewMaxBytes(): Promise<number> {
  ...
  try {
    ...
    return resolvePreviewCacheBudgetBytes(profile, available);
  } catch {
    return profile.proxy.cacheMaxBytes;
  }
}

function isPreviewDerivative(name: string): boolean {
  return /\.(?:peaks\.json|strip\.jpg|frame-\d+\.jpg|poster\.jpg|proxy\.mp4|proxy-status\.json|proxy-[a-z0-9_-]+\.(?:mp4|json))$/i.test(name);
}
...
/** A power loss can leave atomic-build temporary files behind. Only remove
 * UUID-named files inside the private preview cache, and only after a full day
 * so an unusually long live transcode cannot be mistaken for an orphan. */
async function cleanupOrphanPreviewTemps(now = Date.now()): Promise<number> {
  let removed = 0;
  for (const root of uploadReadDirs()) {
    const dir = previewDir(root);
    const names = await readdir(dir).catch(() => [] as string[]);
    for (const name of names) {
      if (!ORPHAN_PREVIEW_TEMP.test(name)) continue;
      const path = join(dir, name);
      const info = await stat(path).catch(() => null);
      if (!info?.isFile() || now - info.mtimeMs < ORPHAN_PREVIEW_TEMP_AGE_MS) continue;
      try {
        await unlink(path);
        removed += 1;
...
async function prunePreviewCache(protectedPaths: ReadonlySet<string> = new Set()): Promise<void> {
  ...
  const victims = selectPreviewEvictions(await previewEntries(), await previewMaxBytes(), protectedAll);
...
async function runDerivative<T>(
  ...
  protectedPath = key,
  queue: DerivativeQueue = derivativeQueue,
): Promise<T> {
  const lease = queue.acquire(key, async (signal) => {
    activePreviewPaths.add(protectedPath);
    try {
      await prunePreviewCache(new Set([protectedPath]));
      return await work(signal);
    } finally {
      await prunePreviewCache(new Set([protectedPath])).catch(() => {});
...
export function mediaPreviewPlugin(): Plugin {
  return {
    ...
    configureServer(server) {
      ...
      void cleanupOrphanPreviewTemps().then(() => prunePreviewCache());
      ...
      server.middlewares.use('/api/media-performance-profile', async (req, res) => {
        if (req.method !== 'GET') {
          sendJson(res, 405, { error: 'method not allowed — use GET' });
          return;
        }
        try {
          sendJson(res, 200, await resolveMediaPerformanceProfile());
        } catch (error) {
          handleDerivativeError(res, logError, 'media-performance-profile', error);
        }
      });
