...
const EXPORT_JOB_FILE_PREFIX = 'yolocut-export-job-';
...
const EXPORT_JOB_FILENAME = /^yolocut-export-job-([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})\.(?:mp4|webm|mov|mp3|wav)$/i;
...
export function resolveMaxActiveExports(value = process.env.YOLOCUT_MAX_ACTIVE_EXPORTS): number {
...
export async function retimeFps(
  ...
  try {
    ...
    return await transformH264(base, input, output, [`fps=${targetFps}`], targetBitrate, 'FPS conversion', signal);
  } catch (error) {
    await unlink(output).catch(() => {});
    throw error;
  }
...
/** Resize an already rendered timeline to the exact requested even-pixel
 * dimensions. This is required when Remotion cannot express the target with a
 * single integer-producing composition scale. */
export async function resizeVideo(
  ...
  if (!Number.isInteger(width) || !Number.isInteger(height) || width < 2 || height < 2
    ...
    throw new Error('ffmpeg resize requires positive even integer dimensions');
  }
  await unlink(output).catch(() => {});
  const base = ['-nostdin', '-hide_banner', '-loglevel', 'error', '-y'];
  ...
  try {
    ...
    return await transformH264(base, input, output, [filter], targetBitrate, 'resolution conform', signal);
  } catch (error) {
    await unlink(output).catch(() => {});
    throw new Error(`ffmpeg resize failed: ${error instanceof Error ? error.message : String(error)}`, { cause: error });
  }
}

async function transformH264(
  base: string[],
  input: string,
  output: string,
  filters: string[],
  ...
  for (const encoder of h264EncoderAttempts(preferred)) {
    try {
      const args = [
        ...
        '-i', input,
        '-vf', h264FilterChain(encoder, filters),
      ...
      await unlink(output).catch(() => {});
      console.warn(`[export] ${encoder} failed during ${operation}; falling back to libx264`);
    }
  }
