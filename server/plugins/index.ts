...
import { localVoiceModelsPlugin } from './local-voice-models.ts';
import { localAutoEditVlmPlugin } from './local-auto-edit-vlm.ts';
import { generateLocalVoiceAudio } from './local-voice-runtime.ts';
...
import { zcodePlugin } from "./zcode.ts";
...
export function serverPlugins(options: { projectStoreHttp?: boolean } = {}): Plugin[] {
  ...
  return [
    ...
    asrModelsPlugin(),
    modelPacksPlugin(),
    localAutoEditVlmPlugin(),
    localVoiceModelsPlugin(),
    ...
    voiceGenerationPlugin({
      ...
      ai: aiVoiceOptions(),
      localVoiceGenerator: generateLocalVoiceAudio,
    }),
