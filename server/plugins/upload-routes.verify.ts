...
const directory = await mkdtemp(join(tmpdir(), 'yolocut-upload-routes-'));
...
try {
  ...
  server = await createServer({
    ...
    // This is an HTTP route test; no source file is ever hot-reloaded. Disable
    // Vite's filesystem watcher so Windows/libuv cannot observe temp-directory
    // 8.3 aliases that differ from the long root path and abort the process.
  ...
  assert.equal(basename(directory).startsWith('yolocut-upload-routes-'), true);
} finally {
  await server?.close();
