...
import type { LocalVoiceDevicePreference, LocalVoiceGenerationMetadata } from '../../shared/local-voice.ts';

export type VoiceProvider =
  | 'elevenlabs' | 'doubao' | 'minimax' | 'inworld' | 'fishaudio' | 'speechify'
  | 'openai' | 'gemini' | 'mistral' | 'cartesia' | 'local';
...
export interface VoiceOptions {
  ...
  ai: AiVoiceOptions;
  /** Injected by the separately reviewed local inference/model-manager backend. */
  localVoiceGenerator?: LocalVoiceGenerator;
}
...
export interface LocalVoiceAudio extends LocalVoiceGenerationMetadata {
  readonly bytes: Buffer;
  readonly codec: 'wav';
  readonly sampleRate: number;
}

export type LocalVoiceGenerator = (input: ValidVoiceRequest) => Promise<LocalVoiceAudio>;
