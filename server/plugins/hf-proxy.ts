...
// The largest pinned pack is Fish S2 Pro Q8_0 (5.63 GB). Keep a fixed ceiling
// above that exact manifest while still rejecting arbitrary oversized files.
const MAX_CACHE_FILE_BYTES = 6 * 1024 * 1024 * 1024; // 6 GiB hard cap
...
const SOURCES: ReadonlyArray<{ name: string; url: (target: ProxyTarget) => string }> = [
  {
    name: 'modelscope',
    url: (target) => `https://modelscope.cn/api/v1/models/${target.modelId}/repo?Revision=${target.revision}&FilePath=${target.filePath}`,
  },
  {
    name: 'huggingface',
...
/** Frozen source URLs for regression checks; never includes credentials. */
export function downloadSourceUrlsForVerify(target: ProxyTarget): readonly { name: string; url: string }[] {
  return SOURCES.map((source) => ({ name: source.name, url: source.url(target) }));
}
...
export function modelCacheDir(): string {
  return join(resolveRuntimeProfile().rootDir, 'asr-models');
}
...
interface FixedCatalogFile {
  readonly integrity: ModelPackFile | AsrModelFile;
  readonly installedModelId: string;
  readonly installedPath: string;
}

function fixedCatalogFile(target: ProxyTarget): FixedCatalogFile | undefined {
  for (const pack of MODEL_PACKS) {
    for (const file of pack.files) {
      ...
      if (source.modelId === target.modelId
        ...
        && source.path === target.filePath) {
        return { integrity: file, installedModelId: pack.modelId, installedPath: file.path };
      }
    }
  }
  const asrFile = asrModelFile(target.modelId, target.revision, target.filePath);
  return asrFile
    ? { integrity: asrFile, installedModelId: target.modelId, installedPath: asrFile.path }
    : undefined;
}

function fixedModelFile(target: ProxyTarget): ModelPackFile | AsrModelFile | undefined {
  return fixedCatalogFile(target)?.integrity;
}
...
async function openInstalledCatalogFile(target: ProxyTarget): Promise<InstalledModelFile | null> {
  const fixed = fixedCatalogFile(target);
  if (!fixed) return null;
  const cacheRoot = modelCacheDir();
  const packRoot = join(cacheRoot, fixed.installedModelId);
  const candidate = join(packRoot, ...fixed.installedPath.split('/'));
  const verified = await openContainedVerifiedFile(cacheRoot, packRoot, candidate, fixed.integrity);
