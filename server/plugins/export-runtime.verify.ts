...
const exportDir = await mkdtemp(join(tmpdir(), 'yolocut-export-cleanup-'));
try {
  const now = Date.now();
  ...
  const prefixedUserName = 'yolocut-export-job-project.mp4';
  await Promise.all([
...
const staleOutput = join(tmpdir(), `yolocut-retime-check-${randomUUID()}.mp4`);
await writeFile(staleOutput, 'stale partial output');
await assert.rejects(
  retimeFps('/definitely/missing/yolocut-input.mp4', staleOutput, 30, 'vp8', 4_000_000),
  /ffmpeg fps retime failed/,
);
assert.equal(existsSync(staleOutput), false, 'failed FPS conversion must remove partial output');

const staleResizeOutput = join(tmpdir(), `yolocut-resize-check-${randomUUID()}.mp4`);
await writeFile(staleResizeOutput, 'stale partial output');
await assert.rejects(
  resizeVideo('/definitely/missing/yolocut-input.mp4', staleResizeOutput, 854, 480, 'vp8', 4_000_000),
  /ffmpeg resize failed/,
);
assert.equal(existsSync(staleResizeOutput), false, 'failed resolution conform must remove partial output');
