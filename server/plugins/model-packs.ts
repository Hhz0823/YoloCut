...
import { discoverFishS2Runtime } from './fish-s2-runtime-discovery.ts';
...
export function modelPackRoot(pack: ModelPackDefinition): string {
...
async function inspectPack(pack: ModelPackDefinition): Promise<PackInspection> {
  ...
  for (const file of pack.files) {
    try {
      const info = await stat(join(modelPackRoot(pack), file.path));
  ...
  for (const file of pack.files) {
    const actual = await sha256(join(modelPackRoot(pack), file.path));
...
async function catalogEntry(pack: ModelPackDefinition): Promise<ModelPackCatalogEntry> {
  ...
  const runtimeAvailability = pack.runtime?.engine === 's2.cpp'
    ? await discoverFishS2Runtime()
    : pack.analysisRuntime?.engine === 'llama.cpp'
      ? await discoverLlamaCppRuntime()
      : undefined;
  return {
    ...pack,
    status,
...
async function catalog(kind?: ModelPackKind): Promise<readonly ModelPackCatalogEntry[]> {
  const entries = await Promise.all(MODEL_PACKS.filter((pack) => !kind || pack.kind === kind).map(catalogEntry));
  if (kind && kind !== 'voice') return entries;
  const hardware = await probeLocalVoiceHardware();
  return entries.map((entry) => entry.kind === 'voice' && entry.id === hardware.recommendation.packId
    ? {
        ...entry,
        hardwareRecommendation: hardware.recommendation,
        ...(hardware.selectedGpu ? { detectedGpu: hardware.selectedGpu } : {}),
      }
    : entry);
}

/** Shared read model for settings-specific projections such as local voice. */
export async function modelPackCatalog(kind?: ModelPackKind): Promise<readonly ModelPackCatalogEntry[]> {
  await ensureRecovered();
  return catalog(kind);
}

/** Integrity-check a catalog pack before a local runtime is allowed to open it. */
export async function inspectModelPack(pack: ModelPackDefinition): Promise<PackInspection> {
  await ensureRecovered();
  return inspectPack(pack);
}
...
async function stageFile(
  ...
  const source = join(modelPackRoot(pack), file.path);
  ...
  const remote = modelPackFileSource(pack, file);
  await downloadModelFile(
    { modelId: remote.modelId, revision: remote.revision, filePath: remote.path },
    destination,
    { signal, onProgress },
  );
...
async function installStagedPack(pack: ModelPackDefinition, stage: string): Promise<void> {
  await replaceDirectoryAtomically(stage, modelPackRoot(pack), backupRoot(pack));
}
...
async function recoverInterruptedInstall(pack: ModelPackDefinition): Promise<void> {
  await recoverDirectorySwap(modelPackRoot(pack), backupRoot(pack));
}
...
export function modelPackLicenseAcceptanceIssue(
  pack: ModelPackDefinition,
  acceptance: unknown,
): string | undefined {
  if (!pack.licensePolicy) return undefined;
  return acceptance === pack.licensePolicy.acceptanceId
    ? undefined
    : `安装 ${pack.label} 前必须明确接受 ${pack.license}；商业使用需要另行取得书面许可。`;
}

async function startDownload(id: string, licenseAcceptance?: unknown): Promise<ModelPackTask> {
  const pack = modelPackDefinition(id);
  if (!pack) throw new Error(`Unknown model pack: ${id || '(empty)'}`);
  const licenseIssue = modelPackLicenseAcceptanceIssue(pack, licenseAcceptance);
...
async function handleDownloadRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  sendJson(res, 202, {
    task: await startDownload(String(body.id ?? ''), body.licenseAcceptance),
  });
}
...
export async function handleModelPackRequest(
  ...
  if (pathname === '/api/model-packs' && req.method === 'GET') {
    const rawKind = new URL(req.url ?? pathname, 'http://127.0.0.1').searchParams.get('kind');
    if (rawKind && rawKind !== 'analysis' && rawKind !== 'voice') {
      throw new Error('Unknown model pack kind');
    }
    const kind: ModelPackKind | undefined = rawKind === 'analysis' || rawKind === 'voice' ? rawKind : undefined;
    sendJson(res, 200, { packs: await modelPackCatalog(kind) });
    return;
  }
