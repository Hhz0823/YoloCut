...
const HTTP_OPERATIONS = {
  ...
  semanticVectorsUpsert: (request: {
    ...
    semanticVectors: semanticVectorsAvailable()
  ...
  semanticVectorsClear: (request: { scopeId: string }) => ({
    semanticVectors: semanticVectorsAvailable() ? clearSemanticVectors(request.scopeId) : null,
  }),
};

export function projectStorePlugin(options: { http?: boolean } = {}): Plugin {
  return {
    name: 'yolocut-project-store',
    configureServer(server) {
      server.middlewares.use('/api/project-store', async (req, res, next) => {
        // Storage migration: status is read-only (loopback reads allowed),
        // the migration itself is a write and requires a real session. These
        // three routes stay mounted in the packaged desktop shell even though
        // ordinary project-store traffic uses the trusted IPC transport there.
