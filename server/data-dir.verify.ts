...
const fixture = await mkdtemp(join(tmpdir(), 'yolocut-data-dir-'));
try {
  ...
  assert.equal(dataDirPointerPath(home), join(home, '.yolocut', 'data-dir.json'));
  assert.equal(readDataDirPointer(home), null);
  const legacyChosen = join(fixture, 'legacy-chosen');
  await mkdir(join(home, '.chatcut'), { recursive: true });
  await writeFile(join(home, '.chatcut', 'data-dir.json'), JSON.stringify({ version: 1, dataDir: legacyChosen }));
  assert.equal(readDataDirPointer(home), legacyChosen, 'YoloCut adopts the prior data-directory pointer');
  await rm(join(home, '.chatcut'), { recursive: true, force: true });
  const chosen = join(fixture, 'chosen');
  await writeDataDirPointer(chosen, home);
  assert.equal(readDataDirPointer(home), chosen);
  // POSIX mode bits are not enforced or reported consistently by Windows;
  // the production write still requests 0600, while this exact-mode assertion
  // remains meaningful only on POSIX filesystems.
  ...
  assert.equal(
    relocatedMediaDestination(null, join(fixture, 'home', '.yolocut'), defaultUploads),
    defaultUploads,
  );
