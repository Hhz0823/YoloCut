...
const globalRoot = join(homeDir, '.yolocut');
...
const overriddenDefault = resolveRuntimeProfile({
  YOLOCUT_PROJECT_STORE_AUTH_DIR: ` ${customAuth} `,
  YOLOCUT_GENERATION_JOB_STORE: customGeneration,
}, { homeDir, cwd });
...
assert.equal(resolveRuntimeProfile({ YOLOCUT_GENERATION_JOB_STORE: '' }, {
  homeDir,
  cwd,
}).generationJobStore, '');
...
assert.throws(
  () => resolveRuntimeProfile({ YOLOCUT_DEV_PROFILE_ROOT: isolatedRoot }, { homeDir, cwd }),
...
const dataDirHome = mkdtempSync(join(tmpdir(), 'yolocut-runtime-data-dir-'));
try {
  const chosen = join(dataDirHome, 'Saves', 'YoloCut');
  ...
  mkdirSync(join(dataDirHome, '.yolocut'), { recursive: true });
  ...
  assert.equal(
    ...
    join(dataDirHome, '.yolocut', 'dev-profiles', profileAId),
  ...
  assert.equal(defaultRootDir(envDataDir, dataDirHome), join(dataDirHome, '.yolocut'));
  assert.equal(
