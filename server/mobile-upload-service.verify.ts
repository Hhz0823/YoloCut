...
const tempDir = await mkdtemp(join(tmpdir(), 'yolocut-mobile-upload-'));
...
try {
  ...
  assert.match(await page.text(), /YoloCut/);
  ...
  assert.match(await (await fetch(englishSession.urls[0]!)).text(), /Send media to YoloCut/);
