...
const startupDir = await mkdtemp(join(tmpdir(), 'yolocut-env-roundtrip-'));
...
// ── ZCode manual recovery remains loopback-only and keeps its key server-side ──
seedKeystore({
  ...isolatedSeed,
  LLM_ZCODE_API_KEY: 'zcode-server-only-secret',
  LLM_ZCODE_BASE_URL: 'http://127.0.0.1:18081',
  LLM_ZCODE_MODEL: 'gemini-3.7-flash',
} as Record<string, string>);
assert.equal(getKey('LLM_ZCODE_BASE_URL'), 'http://127.0.0.1:18081/v1', 'manual loopback URL is normalized to the API prefix');
const zcodeStatus = keyStatus();
assert.equal(zcodeStatus.keys.LLM_ZCODE_API_KEY.configured, true);
assert.equal(zcodeStatus.models.LLM_ZCODE_BASE_URL, 'http://127.0.0.1:18081/v1');
assert.equal(zcodeStatus.models.LLM_ZCODE_MODEL, 'gemini-3.7-flash');
assert.doesNotMatch(JSON.stringify(zcodeStatus), /zcode-server-only-secret/);
seedKeystore({
  ...isolatedSeed,
  LLM_ZCODE_BASE_URL: 'https://relay.example/v1',
} as Record<string, string>);
assert.equal(getKey('LLM_ZCODE_BASE_URL'), '', 'an unsafe persisted ZCode relay is discarded fail-closed');
...
const MODEL_ROUTING_NAMES = [
  ...
  'YOLOCUT_SKILLS_DIR', // User skill files directory (''=~/.yolocut/skills)
