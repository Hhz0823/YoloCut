...
export function resolveLlmProviderConfig(
  ...
  const resolvedBaseUrl = resolveLlmBaseUrl(provider, get(names.baseUrl), AI_SDK_BASE_URL_FORMAT);
  return {
    provider,
    apiKey: get(names.apiKey),
    baseUrl: provider === 'zcode' ? normalizeZCodeBaseUrl(resolvedBaseUrl) : resolvedBaseUrl,
