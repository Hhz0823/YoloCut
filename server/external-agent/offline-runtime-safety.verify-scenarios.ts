...
async function projectOversizedResult(): Promise<ProjectedScenario> {
  ...
  const isolatedProjectId = `${projectId}-artifact-${crypto.randomUUID().slice(0, 8)}`;
  const isolatedEditorUrl = `http://localhost:5173/#/editor/${isolatedProjectId}`;
  const runtime = await OfflineExternalEditRuntime.create(isolatedProjectId, isolatedEditorUrl, {
    persistence: new MemoryPersistence(projectDoc(), isolatedProjectId),
    isBrowserConnected: () => false,
...
async function verifyArtifactRecovery(scenario: ProjectedScenario): Promise<void> {
  ...
  const artifact = await loadAgentArtifact(scenario.projectId, scenario.artifactId);
  assert(artifact);
...
async function verifyProjectionLedger(scenario: ProjectedScenario): Promise<void> {
  ...
  const run = (await loadAgentRuntimeSidecar(scenario.projectId)).runs.find((row) => row.runId === runId);
  assert(run);
  assert.equal(
    run.events.find(
