...
try {
  ...
  const boundA = await connectClient(mcpUrl, 'yolocut-mcp-binding-a');
  clients.push(boundA);
  const connectionTools = (await boundA.client.listTools()).tools.map((tool) => tool.name);
  assert.ok(connectionTools.includes(MCP_STATUS_TOOL), 'new YoloCut clients receive the branded status tool');
  for (const legacyTool of LEGACY_MCP_STATUS_TOOLS) {
    assert.ok(connectionTools.includes(legacyTool), 'legacy clients retain their status alias');
  }
  const brandedStatus = await boundA.client.callTool({ name: MCP_STATUS_TOOL, arguments: {} });
  assert.notEqual(brandedStatus.isError, true);
  for (const legacyTool of LEGACY_MCP_STATUS_TOOLS) {
    const legacyStatus = await boundA.client.callTool({ name: legacyTool, arguments: {} });
    assert.notEqual(legacyStatus.isError, true);
    assert.deepEqual(brandedStatus.structuredContent, legacyStatus.structuredContent);
  }
  let notify!: () => void;
  ...
  const exposureHeaders = { 'x-yolocut-tool-exposure': 'progressive' };
  const progressiveA = await connectClient(mcpUrl, 'yolocut-progressive-a', exposureHeaders);
  const progressiveB = await connectClient(mcpUrl, 'yolocut-progressive-b', exposureHeaders);
  ...
  const boundB = await connectClient(mcpUrl, 'yolocut-mcp-binding-b');
  clients.push(boundB);
  ...
  const switchClient = await connectClient(mcpUrl, 'yolocut-mcp-switch');
  ...
  const closeClientConnection = await connectClient(mcpUrl, 'yolocut-mcp-close');
  ...
  const expiredClient = await connectClient(mcpUrl, 'yolocut-mcp-expired');
  ...
  for (let index = 0; index < MCP_SESSION_COUNT_LIMIT; index += 1) {
    cappedClients.push(await connectClient(mcpUrl, `yolocut-mcp-cap-${index}`));
  }
  ...
  cappedClients.push(await connectClient(mcpUrl, 'yolocut-mcp-cap-overflow'));
