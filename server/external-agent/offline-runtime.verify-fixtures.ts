...
export class MemoryPersistence implements OfflineEditPersistence {
  ...
  constructor(doc: ProjectDoc, expectedProjectId = projectId) {
    this.current = structuredClone(doc);
    this.expectedProjectId = expectedProjectId;
  }

  async claimProject(id: string, ownerId: string) {
    if (id !== this.expectedProjectId || this.ownership) {
      return { status: id === this.expectedProjectId ? 'busy' as const : 'missing' as const };
    }
  ...
  async loadCheckpoint(
    ...
    if (id !== this.expectedProjectId || this.checkpoint?.baseRevision !== expectedRevision) return null;
  ...
  async saveCheckpoint(input: OfflineCheckpointSaveInput) {
    if (!this.matches(input.ownership)
      || input.projectId !== this.expectedProjectId
  ...
  async deleteCheckpoint(
    ...
    if (this.matches(ownership) && id === this.expectedProjectId && this.checkpoint?.sessionId === sessionId) {
      this.checkpoint = null;
    }
  }
