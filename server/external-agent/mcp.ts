...
import {
  buildMcpConnectionManifest,
  MCP_BROWSER_REQUIRED_FOR,
  type McpConnectionManifest,
...
const PROJECT_SELECTOR = {
  type: 'string',
  description: `${PRODUCT_NAME} project id. It must match the project bound to this MCP transport session.`,
};
...
function mcpStatus(session: McpSession): Record<string, unknown> {
  ...
  const manifest = mcpConnectionManifest(session);
  return {
    ...
    capabilityCoverage: manifest.capabilityCoverage,
    fullEditingReadiness: manifest.readiness.fullEditing,
  };
}

export function mcpConnectionManifest(session?: McpSession): McpConnectionManifest {
  const connected = connectedProjectIds();
  const editors = editorStatuses();
  const fullTools = fullMcpTools(session);
  const currentTools = session ? mcpTools(session) : fullTools;
  return buildMcpConnectionManifest({
    skillBaseline: YOLOCUT_SKILL_BASELINE,
    connectedProjectIds: connected,
    editors,
    registeredToolNames: registeredTools().map((tool) => tool.name),
    currentToolNames: currentTools.map((tool) => tool.name),
    fullToolNames: fullTools.map((tool) => tool.name),
    exposureMode: session?.exposure.mode ?? 'full',
    bindingMode: session ? bindingMode(session) : null,
    sessionBinding: session?.binding ?? session?.offline?.binding() ?? null,
  });
}
...
async function callControlTool(
  ...
  if (isCompatibleMcpStatusTool(name)) return mcpStatus(session);
  if (name === 'get_connection_manifest') return mcpConnectionManifest(session);
...
function makeServer(baseUrl: string, session: McpSession): Server {
  const server = new Server(
    { name: MCP_PROTOCOL_SERVER_NAME, version: '1.0.0' },
    {
