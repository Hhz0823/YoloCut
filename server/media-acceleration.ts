...
/** A listed filter is not enough to prove that a driver/device works, but it
 * does prove that the packaged FFmpeg can construct the CUDA zero-copy graph.
 * Runtime failures are still handled by the ordered proxy fallbacks. */
export function ffmpegFilterAvailable(ffmpeg: string, filter: string): Promise<boolean> {
  const safeFilter = /^[a-z0-9_]{1,64}$/i.test(filter) ? filter : '';
  if (!safeFilter) return Promise.resolve(false);
  const key = `${ffmpeg}\0${safeFilter}`;
  ...
  child.once('close', (code) => {
    clearTimeout(timer);
    resolve(code === 0 && new RegExp(`\\bFilter ${safeFilter}\\b`, 'i').test(output));
  });
...
export function resolveVaapiDevice(
  value = process.env.YOLOCUT_VAAPI_DEVICE,
): string {
...
function disabledByEnvironment(): boolean {
  return /^(?:1|true|yes)$/i.test(process.env.YOLOCUT_DISABLE_HARDWARE_ENCODING ?? '');
}
...
export function resolveH264Encoder(
  ...
  const forcedValue = process.env.YOLOCUT_H264_ENCODER?.trim();
