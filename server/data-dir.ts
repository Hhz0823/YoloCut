...
export function dataDirPointerPath(home: string = homedir()): string {
  return join(home, '.yolocut', 'data-dir.json');
}

function legacyDataDirPointerPaths(home: string): string[] {
  return LEGACY_HOME_DIR_NAMES.map((directory) => join(home, directory, 'data-dir.json'));
}
...
export function readDataDirPointer(home: string = homedir()): string | null {
  const current = dataDirPointerPath(home);
  const pointer = existsSync(current)
    ? current
    : legacyDataDirPointerPaths(home).find((candidate) => existsSync(candidate));
