...
export const NON_SECRET_NAMES: ReadonlySet<string> = new Set([
  ...
  "YOLOCUT_SKILLS_DIR", // User skill files directory ('' = ~/.yolocut/skills) - configuration is not credentials
...
function normalizeStoredValue(name: string, raw: unknown): string {
  ...
  if (name === "LLM_ZCODE_BASE_URL" && value) return normalizeZCodeBaseUrl(value);
...
export function seedKeystore(env: Record<string, string>): void {
  for (const name of KEY_NAMES) {
    ...
    try {
      ...
      if (name === MODEL_CAPABILITY_OVERRIDES_KEY || name === "LLM_ZCODE_BASE_URL") {
        store.delete(name);
