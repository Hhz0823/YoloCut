...
import { PRODUCT_DATA_DIR_ENV } from '../shared/product-brand.ts';
...
export const DEV_PROFILE_ID_ENV = 'YOLOCUT_DEV_PROFILE_ID';
export const DATA_DIR_ENV = PRODUCT_DATA_DIR_ENV;
const DEV_PROFILE_ENV_PREFIX = 'YOLOCUT_DEV_PROFILE_';
...
export function resolveRuntimeProfile(
  ...
  const compatibleEnv: Record<string, string | undefined> = { ...env };
  applyLegacyEnvironmentAliases(compatibleEnv);
  const home = locations.homeDir ?? homedir();
  const cwd = locations.cwd ?? process.cwd();
  const profileId = configuredProfileId(compatibleEnv);
  const dataDir = configuredDataDir(compatibleEnv, home, profileId !== null);
  if (profileId) {
    // Only an explicit YOLOCUT_DATA_DIR reaches here (the pointer file is
    ...
    const rootDir = dataDir ?? join(home, '.yolocut', 'dev-profiles', profileId);
  ...
  const rootDir = dataDir ?? join(home, '.yolocut');
  const authOverride = compatibleEnv.YOLOCUT_PROJECT_STORE_AUTH_DIR?.trim();
  const generationOverride = compatibleEnv.YOLOCUT_GENERATION_JOB_STORE;
  const base = profileBase(
    rootDir,
...
export function defaultRootDir(
  ...
  return isIsolatedDevProfile(profile)
    ? join(home, '.yolocut', 'dev-profiles', profile.id)
    : join(home, '.yolocut');
}
