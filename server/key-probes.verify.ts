...
const fixtureHome = mkdtempSync(join(tmpdir(), 'yolocut-key-probes-'));
process.env.HOME = fixtureHome;
process.env.USERPROFILE = fixtureHome;
delete process.env.YOLOCUT_DEV_PROFILE_ID;
delete process.env.YOLOCUT_DATA_DIR;
...
{
  const unset = await runProbe('storage/projects', { YOLOCUT_DATA_DIR: '' });
  assert.equal(unset.ok, true);
  assert.match(unset.message, /默认目录 .*\.yolocut/); // machine-dependent absolute path, anchor the tail only

  const relative = await runProbe('storage/projects', { YOLOCUT_DATA_DIR: 'relative/saves' });
  assert.equal(relative.ok, false);
  assert.match(relative.message, /绝对路径/);
  assert.doesNotMatch(relative.message, /HTTP/);

  const target = join(fixtureHome, 'Saves');
  const writable = await runProbe('storage/projects', { YOLOCUT_DATA_DIR: target });
  ...
  mkdirSync(join(fixtureHome, '.yolocut'), { recursive: true });
  writeFileSync(
    join(fixtureHome, '.yolocut', 'data-dir.json'),
    JSON.stringify({ version: 1, dataDir: target }),
  );
  const recorded = await runProbe('storage/projects', {});
  assert.equal(recorded.ok, true);
  assert.ok(recorded.message.includes(`目录可写 · ${target}`),
    'recorded Windows paths are compared literally rather than parsed as regex escapes');

  process.env.YOLOCUT_DATA_DIR = target;
  try {
    const pinned = await runProbe('storage/projects', { YOLOCUT_DATA_DIR: join(fixtureHome, 'Elsewhere') });
