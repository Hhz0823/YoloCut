// User-visible skill files under ~/.yolocut/skills/<slug>/SKILL.md —
...
/** Override for tests and unusual installs; defaults to ~/.yolocut/skills. */
export function skillFilesRoot(): string {
  return getKey('YOLOCUT_SKILLS_DIR') || join(runtimeProfile().rootDir, 'skills');
}
...
/** Scan ~/.yolocut/skills/ for SKILL.md files (user-dropped or mirrored). */
...
/** Mirror one skill to ~/.yolocut/skills/<slug>/SKILL.md (idempotent). */
...
/** Human-readable install location for tool responses: ~/.yolocut/skills/<slug>. */
export function displaySkillPath(slug: string): string {
  return join('~', '.yolocut', 'skills', slug);
}
