...
function browserFailureSummary(result: unknown): string {
  ...
  return 'YoloCut tool execution failed.';
}
...
function baseInstructions(request: CodexTurnRequest): string {
  const mode = request.askOnly
    ? [
        ...
        'You may call the provided read-only YoloCut tools when current skill or project evidence is needed.',
      ].join(' ')
    : [
        'Use only the provided YoloCut editor tools when the task needs project reads or changes.',
        'Their results come from the existing YoloCut proposal workflow; never claim a change succeeded before its tool result confirms success.',
      ].join(' ');
  return [
    request.system.trim(),
    'YoloCut runtime rules:',
...
export class CodexTurnManager {
  ...
  private serverRequest(session: TurnSession, request: CodexServerRequest): boolean {
    ...
    if (!callId || !name || !session.toolNames.has(name) || session.pendingTools.has(callId)) {
      const message = 'This YoloCut tool call is unavailable. It was not part of this request (stale tool list, duplicate call, or malformed id). Tell the user to open the project and retry; if it persists, start a new run.'
  ...
  private abortPendingTools(session: TurnSession): void {
    for (const pending of session.pendingTools.values()) {
      pending.request.respond({
        contentItems: [{ type: 'inputText', text: 'YoloCut cancelled this tool call.' }],
        success: false,
      });
    }
