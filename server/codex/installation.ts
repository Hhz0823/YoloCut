...
function configuredCandidates(): string[] {
  const configured = process.env.YOLOCUT_CODEX_PATH?.trim();
...
async function readVersion(path: string): Promise<string | null> {
  ...
  try {
    ...
    // Windows Store aliases can be discoverable regular files while process
    // creation is denied with a synchronous EPERM. Status discovery must
    // degrade to an unsupported installation instead of failing the endpoint.
    resolve(null);
  }
  return promise;
}
