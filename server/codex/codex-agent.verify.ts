...
const installationSource = await readFile(new URL('./installation.ts', import.meta.url), 'utf8');
assert.match(
  installationSource,
  /try\s*\{\s*execFile\([\s\S]*?\}\s*catch\s*\{[\s\S]*?resolve\(null\)/,
  'a synchronously denied Windows Store Codex alias degrades to an unavailable status',
);
...
const directory = await mkdtemp(join(tmpdir(), 'yolocut-codex-verify-'));
...
try {
  ...
  assert.deepEqual(events[0], {
    ...
    result: { error: 'This YoloCut tool call is unavailable. It was not part of this request (stale tool list, duplicate call, or malformed id). Tell the user to open the project and retry; if it persists, start a new run.' },
    success: false,
  });
