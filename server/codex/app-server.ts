...
const CODEX_HOME = join(runtimeProfile().rootDir, 'codex');
...
export class CodexAppServerClient {
  ...
  private async startProcess(): Promise<void> {
    ...
    const runtimeCwd = await mkdtemp(join(tmpdir(), 'yolocut-codex-runtime-'));
    ...
    try {
      await this.rawRequest('initialize', {
        clientInfo: { name: 'yolocut', title: 'YoloCut', version: '1' },
  ...
  private handleServerRequest(id: number | string, method: string, params: Record<string, unknown>): void {
    ...
    if (method === 'execCommandApproval' || method === 'applyPatchApproval') {
      this.writeMessage({ id, result: { decision: { denied: { rejection: 'Use the YoloCut proposal review.' } } } });
      return;
    }
    ...
    for (const handler of this.serverRequests) {
      try {
        ...
        request.reject(-32603, 'YoloCut could not handle this request.');
        return;
      }
    }
    request.reject(-32601, 'Method not supported by YoloCut.');
  }
