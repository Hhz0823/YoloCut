...
export function uploadReadDirs(
  profile: RuntimeProfile = runtimeProfile(),
  configuredMediaDir?: string,
  legacyMediaDir: string | undefined = process.env[LEGACY_MEDIA_DIR_ENV],
): readonly string[] {
  const writable = uploadDir(profile, configuredMediaDir);
  if (isIsolatedDevProfile(profile)) return [writable];
  const roots = writable === profile.mediaDir ? [writable] : [writable, profile.mediaDir];
  const legacy = legacyMediaDir ? expandMediaDir(legacyMediaDir) : null;
  if (legacy && !roots.some((root) => resolve(root) === legacy)) roots.push(legacy);
  return roots;
}
