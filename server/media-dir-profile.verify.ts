...
const fixture = await mkdtemp(join(tmpdir(), 'yolocut-media-profile-'));
try {
  ...
  const legacyMedia = join(fixture, 'legacy-yolocut-media');
  assert.deepEqual(
    uploadReadDirs(defaultProfile, '', legacyMedia),
    [defaultProfile.mediaDir, legacyMedia],
    'renamed YoloCut builds retain a non-destructive legacy media read root',
  );
  ...
  const profileA = resolveRuntimeProfile({
    YOLOCUT_DEV_PROFILE_ID: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
  }, { homeDir, cwd });
  const profileB = resolveRuntimeProfile({
    YOLOCUT_DEV_PROFILE_ID: 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb',
  }, { homeDir, cwd });
  ...
  assert.deepEqual(
    uploadReadDirs(profileA, customDir, legacyMedia),
    [profileA.mediaDir],
    'isolated development profiles never leak the packaged legacy media root',
  );
