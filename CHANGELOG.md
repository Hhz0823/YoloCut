...
All notable changes to YoloCut are documented here.
YoloCut 的重要变更记录在此。
...
- The product, desktop executable, package identity, MCP endpoint, skills, documentation, and release artifacts are now consistently branded as YoloCut v0.0.1.
  产品、桌面可执行文件、包标识、MCP 接入、技能、文档与发布构建物现已统一为 YoloCut v0.0.1。
- A centralized compatibility boundary migrates legacy settings, profiles, model/runtime directories, storage keys, project formats, ports, and MCP tokens without duplicating large media or model files.
  新增集中式兼容边界，可迁移旧设置、用户配置、模型/运行时目录、存储键、工程格式、端口与 MCP 令牌，同时避免复制大型媒体或模型文件。

- The preview canvas now supports fit-relative zoom from 25% to 400%, Ctrl/Cmd-wheel zooming, and a bounded hand-pan mode without changing rendered output.
  预览画布新增相对适配视图的 25%–400% 缩放、Ctrl/Cmd+滚轮缩放与有边界的抓手平移模式，不影响最终渲染。
- Canvas settings now accept validated custom dimensions from 16 to 8192 pixels and provide title/action, rule-of-thirds, TikTok, Reels, Shorts, and Spotlight composition guides.
  画布设置新增 16–8192 像素的自定义尺寸，并提供标题/动作安全区、三分构图及 TikTok、Reels、Shorts、Spotlight 构图参考线。
- Keyframed properties now expose a visual cubic-Bezier editor with draggable control points, numeric coordinates, easing presets, live preview, and single-gesture undo behavior.
  关键帧属性新增可视化三次贝塞尔编辑器，支持拖动控制点、数值坐标、缓动预设、实时预览与单手势撤销。
- Production builds now enforce separate static startup budgets for the bootstrap, dashboard, transcript, detached Agent, and full editor renderer surfaces.
  生产构建新增启动体积门禁，分别约束引导层、工程首页、文字稿窗口、独立 Agent 窗口与完整编辑器的静态依赖图。
- Desktop acceptance now drives a real Agent workbench round trip: detach, dock left, detach again, and dock right, while checking the native heartbeat policy and window cleanup.
  桌面验收新增真实 Agent 工作台往返：拖出、停靠左侧、再次拖出、停靠右侧，并检查原生心跳策略与窗口清理。

### Changed / 变更

- The detached Agent workbench now defaults to a compact 860×640 landscape workspace while retaining a 380×480 low-resolution minimum.
  独立 Agent 工作台默认改为紧凑的 860×640 横向工作区，同时保留 380×480 的低分辨率最小尺寸。

- The main, transcript, and detached Agent windows now load only their own renderer graph; dashboard dialogs and first-run templates load on demand. The HEIC decoder is deferred until an actual HEIC/HEIF phone import, reducing the shared editor startup chunk from about 3.50 MB to about 510 KB in the verified production build.
  主窗口、文字稿窗口与独立 Agent 窗口现在只加载各自的渲染依赖；首页弹窗和首次示例模板按需加载。HEIC 解码器仅在真实 HEIC/HEIF 手机导入时加载，已验证生产构建中的共享编辑器启动分包由约 3.50 MB 降至约 510 KB。

### Fixed / 修复

- Windows Agent tear-off testing now uses trusted Chromium pointer input, and mouse dragging no longer takes premature pointer capture that could interrupt the Electron move stream.
  Windows Agent 拖出验收改用受信任的 Chromium 指针输入，鼠标拖动不再过早捕获指针而中断 Electron 移动事件流。
- Model-file verification now fingerprints nanosecond metadata and rechecks the open file after hashing, preventing a same-size rapid replacement from reusing a stale integrity result.
  模型文件校验现使用纳秒级元数据指纹，并在哈希后复查已打开文件，避免同尺寸文件被快速替换时复用过期完整性结果。

- Vite warmup entries now use filesystem-relative paths, eliminating the recurring Windows `/@fs/src/main.tsx` pre-transform error without eagerly compiling the full editor graph.
  Vite 预热入口改用文件系统相对路径，消除 Windows 上反复出现的 `/@fs/src/main.tsx` 预转换错误，同时避免提前编译完整编辑器依赖图。

### Attribution / 来源说明

- YoloCut v0.0.1 continues from the AGPL-licensed OpenChatCut codebase. A
  centralized migration layer recognizes the former storage and MCP aliases so
  existing projects and external agents keep working.
  YoloCut v0.0.1 基于 AGPL 许可的 OpenChatCut 代码继续开发；集中式迁移层继续识别
  旧数据目录及 MCP 别名，保证已有工程与外部 Agent 可继续使用。

## Upstream OpenChatCut history / 上游 OpenChatCut 历史

The entries below are retained from the upstream project for license and change-history continuity.
以下条目保留自上游项目，用于维持许可证与变更历史的连续性。
...
- Added dashboard header shortcuts for contacting the author and opening the YoloCut GitHub repository; the contact disclosure shows a selectable email link without leaving the project list.
  首页顶栏新增“联系作者”和 GitHub 仓库快捷入口；联系信息会就地显示可选择的邮箱链接，无需离开工程列表。
- Added opt-in blurred background fill for video and image clips: the Inspector offers exact 0–100% intensity control plus four quick shortcuts, while `edit_item` accepts `backgroundFillStrength`. The sharp foreground remains independently movable, resizable, croppable, and rotatable. Shared preview/export compositing preserves fades, effects, and GLSL transition alpha; FCPXML retains the toggle and percentage as YoloCut metadata and explicitly reports that destination editors cannot reconstruct the generated blur layer from those custom fields.
  新增视频与图片片段的可选模糊背景填充：检查器支持 0–100% 精确强度调节和四个快捷档位，`edit_item` 接受 `backgroundFillStrength`；清晰前景框仍可独立移动、缩放、裁剪和旋转。预览与导出共用合成链并保留淡化、特效和 GLSL 转场透明度；FCPXML 会把开关与百分比保存在 YoloCut 自定义元数据中，并明确提示目标剪辑软件无法从这些字段自动重建生成的模糊图层。
...
- Added `install_skill`: the Agent can install a complete GitHub skill repo (SKILL.md + references/scripts/assets/examples) into `~/.yolocut/skills/<slug>/`, with GitHub API rate-limit fallback to a shallow git clone and `GITHUB_TOKEN` support.
  新增 `install_skill`：Agent 可把完整 GitHub 技能仓库（SKILL.md + references/scripts/assets/examples）安装到 `~/.yolocut/skills/<slug>/`，GitHub API 限流时自动回退浅克隆，并支持 `GITHUB_TOKEN`。
...
- Added a user-visible custom-skill directory mirroring `~/.codex/skills` / `~/.claude/skills`: `~/.yolocut/skills/<slug>/SKILL.md` (Windows `%USERPROFILE%\.yolocut\skills\...`). `manage_skill create` installs there, hand-dropped SKILL.md files are discovered automatically, and the bundled *skill-creator* workflow guides skill authoring.
  新增用户可见的自定义技能目录，与 `~/.codex/skills` / `~/.claude/skills` 布局一致：`~/.yolocut/skills/<slug>/SKILL.md`（Windows 为 `%USERPROFILE%\.yolocut\skills\...`）。`manage_skill create` 安装到此目录，手动放入的 SKILL.md 会被自动发现，内置“技能创作器”工作流可引导技能编写。
...
- Initial public release of the local-first, agent-native YoloCut video editor.
  首次公开发布 local-first、agent-native 的 YoloCut 视频编辑器。
